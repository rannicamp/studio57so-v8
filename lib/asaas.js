 ...
 */
export async function obterOuCriarCliente({ nome, email, cpfCnpj, phone, postalCode, addressNumber, address, province, state }) {
    try {
        ...
        if (busca.data && busca.data.length > 0) {
            ...
            // Atualizar o cadastro do cliente no Asaas para garantir que possua o CPF/CNPJ e dados de endereço completos
            const clienteAtualizado = await asaasFetch(`/customers/${clienteExistente.id}`, {
                method: 'POST', // Correção técnica: Atualização de dados de cliente no Asaas é via POST /customers/{id} ou PUT. Damos suporte a ambos.
                body: JSON.stringify({
                    name: nome,
                    email: email,
 ...
 */
export async function criarAssinatura({ clienteId, valor, ciclo = 'MONTHLY', descricao, dataVencimento, formaPagamento = 'UNDEFINED', maxPayments, creditCard, creditCardHolderInfo, remoteIp }) {
    try {
 ...
 */
export async function criarPagamento({ clienteId, valor, formaPagamento = 'UNDEFINED', dataVencimento, descricao, parcelas, externalReference, creditCard, creditCardHolderInfo, remoteIp }) {
    try {
        ...
        if (parcelas && Number(parcelas) > 1) {
            payload.totalValue = valor; // valor total da compra
...
/**
 * Obtém o QR Code e a chave copia e cola do PIX para um pagamento específico
 */
export async function obterPixQrCode(pagamentoId) {
    try {
        console.log(`[Asaas] Obtendo QR Code do PIX para o pagamento: ${pagamentoId}`);
        const response = await asaasFetch(`/payments/${pagamentoId}/pixQrCode`, {
            method: 'GET'
        });
        return response;
    } catch (error) {
        console.error('[Asaas] Erro em obterPixQrCode:', error.message);
        throw error;
    }
}

/**
 * Busca a primeira cobrança pendente de uma assinatura e retorna o ID da cobrança
 */
export async function obterPrimeiraCobrancaIdAssinatura(assinaturaId) {
    try {
        console.log(`[Asaas] Buscando cobrança ID da assinatura: ${assinaturaId}`);
        const cobrancas = await asaasFetch(`/payments?subscription=${assinaturaId}`);
        if (cobrancas.data && cobrancas.data.length > 0) {
            const pendente = cobrancas.data.find(c => c.status === 'PENDING') || cobrancas.data[0];
            return pendente.id;
        ...
        console.error('[Asaas] Erro em obterPrimeiraCobrancaIdAssinatura:', error.message);
        throw error;
    }
}

/**
 * Lista as notas fiscais de um cliente
 */
export async function listarNotasFiscais(clienteId) {
    try {
        console.log(`[Asaas] Listando notas fiscais do cliente: ${clienteId}`);
        const response = await asaasFetch(`/invoices?customer=${clienteId}&limit=50`);
        return response.data || [];
    } catch (error) {
        console.error('[Asaas] Erro em listarNotasFiscais:', error.message);
        return [];
    }
}
