    ...
    LOOP
        IF v_prev_dir IS NOT NULL AND v_prev_dir != rec.direction THEN
            ...
            IF v_prev_dir = 'inbound' AND rec.direction = 'outbound' THEN
                v_broker_sum_minutes := v_broker_sum_minutes + EXTRACT(EPOCH FROM (rec.created_at - v_prev_msg_time))/60;
            ...
            IF v_prev_dir = 'outbound' AND rec.direction = 'inbound' THEN
                v_lead_sum_minutes := v_lead_sum_minutes + EXTRACT(EPOCH FROM (rec.created_at - v_prev_msg_time))/60;
...
$$;
  ...
  try {
      ...
      console.log('RPC get_conversation_response_kpis criada com sucesso!');
  } catch(e) {
