...
import { faPlus, faEdit, faTrash, faSpinner, faCopy, faSort, faSortUp, faSortDown, faSave, faDollarSign, faTasks } from '@fortawesome/free-solid-svg-icons';
...
export default function ProdutoList({ initialProdutos, empreendimentoId, initialConfig, onUpdate }) {
        ...
        mutationFn: async (newPrecoM2Value) => {
            ...
            const updatePromises = Array.from(selectedProdutos).map(productId => {
                ...
                return supabase
                    ...
                    .update({ preco_m2: newPrecoM2, valor_base: novoValorBase })
                    .eq('id', productId);
            }).filter(Boolean);
    ...
    // =================================================================================
    // LÓGICA DE EDIÇÃO INLINE CORRIGIDA AQUI
    // O PORQUÊ: A lógica anterior estava incompleta e não salvava a alteração do
    // "Fator de Reajuste". Esta versão foi reestruturada para lidar corretamente
    // com todas as edições: Preço/m², Valor Base e o Fator, garantindo que
    // todos os cálculos dependentes sejam refeitos e salvos corretamente.
    // =================================================================================
    const inlineUpdateMutation = useMutation({
        mutationFn: async ({ productId, field, value }) => {
            ...
            // Sempre recalcula o valor de venda final com base nos valores mais atuais
            ...
            if (Object.keys(updatedValues).length === 0) return; // Não faz nada se não houver o que atualizar
    ...
    const renderEditableCell = (produto, field, formatFn) => {
        ...
        if (editingCell?.rowId === produto.id && editingCell?.field === field) {
            if (field === 'status') { return ( <select defaultValue={produto[field]} onBlur={(e) => handleInlineUpdate(produto.id, field, e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === 'Tab') e.target.blur(); if (e.key === 'Escape') setEditingCell(null); }} autoFocus className="p-1 border rounded-md w-full bg-yellow-50"> <option>Disponível</option> <option>Reservado</option> <option>Vendido</option> </select> ); }
            ...
            <div className="flex flex-wrap justify-between items-end gap-4 p-4 bg-gray-50 rounded-lg border">
                <div className={`transition-all duration-300 ${selectedProdutos.size > 0 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                     ...
                     <div className="flex items-center gap-2">
                         <IMaskInput mask="R$ num" blocks={{ num: { mask: Number, thousandsSeparator: '.', radix: ',', scale: 2, padFractionalZeros: true }}} value={bulkPrecoM2} onAccept={(value) => setBulkPrecoM2(value)} className="p-2 border rounded-md shadow-sm w-48" />
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            {/* ================================================================================= */}
                            {/* COLUNA DO FATOR DE REAJUSTE DE VOLTA AO JOGO! */}
                            {/* ================================================================================= */}
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {isMutating && sortedProdutos.length === 0 ? ( <tr><td colSpan="10" className="text-center py-10"><FontAwesomeIcon icon={faSpinner} spin size="2x" /></td></tr>
                            ...
                            sortedProdutos.map(produto => (
                                <tr key={produto.id} className={`hover:bg-gray-50 ${produto.status === 'Vendido' ? 'bg-red-50' : produto.status === 'Reservado' ? 'bg-yellow-50' : ''} ${selectedProdutos.has(produto.id) ? 'bg-blue-50' : ''}`}>
                                    ...
                                    <td className="px-4 py-2 text-right font-semibold bg-blue-50">{renderEditableCell(produto, 'preco_m2', formatCurrency)}</td>
                                    <td className="px-4 py-2 text-right text-gray-600">{renderEditableCell(produto, 'valor_base', formatCurrency)}</td>
                                    {/* ================================================================================= */}
                                    {/* DADO DO FATOR DE REAJUSTE DE VOLTA À TABELA! */}
                                    {/* ================================================================================= */}
                                    ...
                                    <td className="px-4 py-2 text-center space-x-2">
                                        {produto.status === 'Disponível' && (<button onClick={() => handleSellProduct(produto)} title="Vender esta Unidade" className="text-green-600 hover:text-green-800"><FontAwesomeIcon icon={faDollarSign} /></button>)}
