...
export default function AlmoxarifadoManager() {
 ...
 className={`w-28 text-center px-2 py-1 text-xs font-semibold rounded-full transition-all duration-300 ease-in-out transform hover:scale-105 group relative ${item.material.classificacao === 'Equipamento' ? 'bg-blue-600 text-blue-600' : 'bg-teal-100 text-teal-800'}`}
 ...
 <button onClick={() => handleOpenBaixaQuebraModal(item)} className="bg-blue-600 text-white px-3 py-1 rounded-md text-xs hover:bg-blue-600 font-bold">
 Baixa (Quebra)
 </button>
 <button onClick={() => handleOpenHistoricoModal(item)} className="bg-gray-200 text-gray-700 px-3 py-1 rounded-md text-xs hover:bg-gray-300 font-bold" title="Ver Histórico">
 <FontAwesomeIcon icon={faHistory} />
