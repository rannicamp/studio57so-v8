...
export default function MinhasNotificacoes() {
 ...
 // Cria localmente
 return [...(old || []), { template_id: regraId, [canal]: novoStatus, // Mock dos outros valores
 ...
 return Object.keys(grupos).sort().reduce((obj, key) => { obj[key] = grupos[key]; return obj;
 }, {});
 ...
 disabled={disabled}
 />
 <div className={`w-11 h-6 rounded-full peer peer-focus:ring-4 peer-focus:ring-blue-100 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full peer-checked:after:border-white
 ...
 <FontAwesomeIcon icon={faSpinner} spin className="text-2xl" /> <span className="text-sm">Carregando preferências...</span>
 </div>
 );
 ...
 const webAtivo = pref ? pref.canal_sistema : true; const pushAtivo = pref ? pref.canal_push : true;
 ...
 <ToggleSwitch label="Web" icon={faDesktop}
 active={webAtivo} onChange={(val) => toggleChannel.mutate({ regraId: regra.id, canal: 'canal_sistema', novoStatus: val })}
 />
 <ToggleSwitch label="Celular" icon={faMobileScreen}
 active={pushAtivo}
