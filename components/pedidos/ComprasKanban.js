...
export default function ComprasKanban({ pedidos, fases = [], // Agora recebemos as fases como prop
 onCardClick,
 onDeleteAllCanceled, canDelete, isDeleting }) {
 ...
 updateStatusMutation.mutate({ pedidoId: draggedPedido.id, newFaseId: targetFaseId,
 ...
 {fases.map(fase => (
 <div key={fase.id} // Chave é o UUID
 ...
 <PedidoCard pedido={pedido} // Passamos as opções de menu (Texto) para o card, se precisar
 allStatusColumns={fases.map(c => c.nome)} onStatusChange={(pid, newStatus) => {
