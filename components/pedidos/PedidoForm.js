...
const fetchPedidoData = async (supabase, pedidoId, organizacaoId) => {
 ...
 .from('pedidos_compra')
 .select(`
 *, solicitante:solicitante_id(nome), empreendimentos(nome, empresa_id:empresa_proprietaria_id), itens:pedidos_compra_itens(*, fornecedor:fornecedor_id(id, nome, razao_social, nome_fantasia), etapa:etapa_id(nome_etapa)
 ...
 const signedPromises = pedidoData.anexos.map(a => supabase.storage.from('pedidos-anexos').createSignedUrl(a.caminho_arquivo, 3600)
 );
