...
import { faSpinner, faPlus, faEdit, faTimes } from '@fortawesome/free-solid-svg-icons';
...
export default function PedidoItemModal({ isOpen, onClose, onSave, itemToEdit }) {
 ...
 <div className="flex justify-between items-center mb-4 -mx-6 -mt-6 px-6 py-4 bg-blue-600 rounded-t-lg">
 <h3 className="text-lg font-bold text-white">{isEditing ? 'Editar Item do Pedido' : 'Adicionar Item ao Pedido'}</h3>
 ...
 <button onClick={handleResetItemSelection} className="text-blue-600 hover:text-blue-800 text-sm font-semibold flex items-center gap-1"> <FontAwesomeIcon icon={faEdit} /> Alterar </button>
 </div>
 ) : (
 <>
 <input type="text" value={searchTerm} onChange={handleMaterialSearchChange} placeholder="Digite para buscar ou descrever..." className="mt-1 w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500" autoComplete="off" />
 ...
 <button type="button" onClick={onClose} className="font-bold text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 px-4 py-2 rounded-lg transition-colors">Cancelar</button>
 <button onClick={handleSaveClick} className="bg-blue-600 text-white font-bold px-4 py-2 rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors" disabled={isSaving}> {isSaving ? <FontAwesomeIcon icon={faSpinner} spin /> : 'Salvar Item'} </button>
 </div>
 </div>
 </div>
 );
