...
// Baseado no SimuladorFinanceiroPublico.js (cópia fiel)
// Diferença: empreendimento fixado em id=6 (Refúgio Braúnas)
// ============================================================
'use client';
...
// ─── Constantes ────────────────────────────────────────────────
// O empreendimento é FIXO — sempre Refúgio Braúnas
...
export default function SimuladorBraunas() {
    ...
    // Empreendimento fixo — apenas simulamos o objeto para compatibilidade
    ...
    // ─── Busca automática dos produtos do Refúgio Braúnas ──────
    useEffect(() => {
    ...
    const handleSimular = () => {
        ...
        // Parcelas de Entrada (não entram no saldo devedor financiado)
        if (plano.num_parcelas_entrada > 0 && plano.entrada_valor > 0) {
            ...
            for (let i = 0; i < plano.num_parcelas_entrada; i++) {
                ...
                novasParcelas.push({ id: idCounter++, tipo: 'entrada', descricao: `Entrada ${i + 1}/${plano.num_parcelas_entrada}`, data_vencimento: dataVencimento.toISOString().split('T')[0], valor_parcela: valorParcelaEntrada, correcao_igpm: 0 });
            }
        }
        ...
        if (plano.num_parcelas_obra > 0 && plano.parcelas_obra_valor > 0 && plano.data_primeira_parcela_obra) {
            ...
            for (let i = 0; i < plano.num_parcelas_obra; i++) {
                ...
                dataVencimento.setUTCMonth(dataVencimento.getUTCMonth() + i);
                novasParcelas.push({ id: idCounter++, tipo: 'obra', descricao: `Parcela Obra ${i + 1}/${plano.num_parcelas_obra}`, data_vencimento: dataVencimento.toISOString().split('T')[0], valor_parcela: valorParcelaObra, correcao_igpm: 0 });
            }
        }

        parcelasIntermediarias.forEach(p => {
            if (p.data_vencimento && p.valor_parcela > 0) {
                novasParcelas.push({ ...p, id: idCounter++, tipo: 'intermediaria', correcao_igpm: 0 });
            }
        });
        ...
        if (plano.num_parcelas_saldo > 0 && plano.saldo_remanescente_valor > 0 && plano.data_primeira_parcela_saldo) {
            ...
            for (let i = 0; i < plano.num_parcelas_saldo; i++) {
                ...
                dataVencimento.setUTCMonth(dataVencimento.getUTCMonth() + i);
                novasParcelas.push({ id: idCounter++, tipo: 'saldo', descricao: `Saldo Remanescente ${i + 1}/${plano.num_parcelas_saldo}`, data_vencimento: dataVencimento.toISOString().split('T')[0], valor_parcela: valorParcelaSaldo, correcao_igpm: 0 });
            }
        }

        novasParcelas.sort((a, b) => new Date(a.data_vencimento) - new Date(b.data_vencimento));

        // ── CORREÇÃO ANUAL IGP-M SOBRE O SALDO DEVEDOR ─────────────
        // O saldo devedor = tudo exceto a entrada. A cada 12 meses,
        // inserimos uma linha "Correção IGP-M" com o valor calculado.
        const taxaIGPM = indiceIGPM ?? 0;
        if (taxaIGPM > 0) {
            // Saldo devedor inicial = obra + intermediárias + saldo remanescente
            let saldoDevedor = (parseFloat(plano.parcelas_obra_valor) || 0)
                + totalIntermediarias
                + (parseFloat(plano.saldo_remanescente_valor) || 0);

            const parcelasFinanciadas = novasParcelas.filter(p => p.tipo !== 'entrada');
            if (parcelasFinanciadas.length > 0) {
                const dataBase = new Date(parcelasFinanciadas[0].data_vencimento + 'T00:00:00Z');
                const correcoes = [];
                let mesContador = 0;
                ...
                for (const p of novasParcelas) {
                    ...
                    mesContador++;
                    saldoDevedor -= p.valor_parcela;

                    // A cada 12 meses, insere a linha de correção logo após a parcela
                    if (mesContador % 12 === 0 && saldoDevedor > 0.01) {
                        const dataCorrecao = new Date(p.data_vencimento + 'T00:00:00Z');
                        // Coloca 1 dia depois para aparecer logo após a parcela
                        dataCorrecao.setUTCDate(dataCorrecao.getUTCDate() + 1);
                        const valorCorrecao = saldoDevedor * (taxaIGPM / 100);
                        correcoes.push({
                            id: idCounter++,
                            tipo: 'correcao',
                            descricao: `Correção Anual IGP-M (${taxaIGPM.toFixed(2)}% sobre R$ ${saldoDevedor.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})})`,
                            data_vencimento: dataCorrecao.toISOString().split('T')[0],
                            valor_parcela: valorCorrecao,
                            correcao_igpm: valorCorrecao,
                            saldo_base: saldoDevedor,
                            taxa_aplicada: taxaIGPM
                        });
                        // O saldo devedor aumenta com a correção
                        saldoDevedor += valorCorrecao;
                    }
                }
                novasParcelas = [...novasParcelas, ...correcoes].sort(
        ...
        toast.success(`Simulação gerada com ${novasParcelas.filter(p => p.tipo === 'correcao').length} correção(ões) IGP-M inserida(s)!`);
    };
    ...
    const handleEnviarProposta = async () => {
        ...
        const simulacaoCompletaData = {
            ...
            cliente,
            corretor,
            organizacao_id: 1, // Refúgio Braúnas pertence à Org 1
        };
    ...
    return (
        <div>
            <style jsx global>{`@media print {@page { size: A4 portrait; margin: 1cm; } html, body { height: initial !important; overflow: initial !important; -webkit-print-color-adjust: exact; } .printable-area { padding: 0 !important; margin: 0 !important; height: auto; } .no-print { display: none !important; } tfoot { display: table-row-group; }}`}</style>
            ...
            <div className="space-y-8 no-print">
                ...
                <fieldset className="p-4 border rounded-lg">
                    ...
                    {/* Empreendimento fixo — exibição informativa */}
                    ...
                    <div className="flex items-center gap-2">
                        <select value={currentSelectedProdutoId} onChange={(e) => setCurrentSelectedProdutoId(e.target.value)} disabled={loadingProdutos} className="p-2 border rounded-md w-full">
                            ...
                            {produtos.filter(p => !selectedProdutos.some(sp => sp.id === p.id)).map(p => <option key={p.id} value={p.id}>Lote {p.unidade} ({p.tipo}) — {parseFloat(p.area_m2).toLocaleString('pt-BR', {maximumFractionDigits: 0})} m² — {formatCurrency(p.valor_venda_calculado)}</option>)}
                        </select>
                        ...
                        <div className="flex items-center justify-between mb-3">
                            ...
                            {indiceIGPM !== null && (
                                <span className="text-xs bg-amber-50 text-amber-700 border border-amber-200 px-2.5 py-1 rounded-full font-bold">
                                    IGP-M 12M: {isLoadingIGPM ? '...' : `${indiceIGPM.toFixed(2)}%`}
                                </span>
                            )}
                        </div>
                            ...
                            <table className="min-w-full divide-y divide-gray-200 text-sm">
                                <thead className="bg-gray-100">
                                    <tr>
                                        ...
                                        <th className="px-4 py-2 text-right font-semibold">Correção IGP-M</th>
                                    </tr>
                                </thead>
                                <tbody className="bg-white divide-y">
                                    {cronograma.map(p => (
                                        <tr key={p.id} className={p.tipo === 'correcao' ? 'bg-amber-50' : ''}>
                                            ...
                                            <td className="px-4 py-2 text-gray-500">{formatDateForDisplay(p.data_vencimento)}</td>
                                            <td className={`px-4 py-2 text-right font-medium ${p.tipo === 'correcao' ? 'text-amber-700 font-bold' : ''}`}>
                                            ...
                                            <td className="px-4 py-2 text-right">
                                                {p.tipo === 'correcao' ? (
                                                    <span className="text-xs font-bold text-red-500 bg-red-50 px-2 py-0.5 rounded border border-red-100">
                                                        + {formatCurrency(p.correcao_igpm)}
                                                    </span>
                                                ) : (
