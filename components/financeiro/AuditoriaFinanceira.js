...
import { faCheckCircle, faTimesCircle, faExclamationTriangle, faSearch, faSpinner, faFileInvoiceDollar, faEye } from '@fortawesome/free-solid-svg-icons';
...
export default function AuditoriaFinanceira() {
 ...
 e.stopPropagation(); auditMutation.mutate(id);
 };
 ...
 <button onClick={() => queryClient.invalidateQueries({ queryKey: ['lancamentosAuditoria'] })}
 ...
 <tr key={lancamento.id} onClick={() => handleRowClick(lancamento)}
 ...
 ${auditandoId === lancamento.id ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 text-white hover:shadow-md'
 }
 `}
 >
