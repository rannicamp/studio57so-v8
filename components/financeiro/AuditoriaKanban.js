...
const formatarDataVisual = (dataString) => {
 ...
 const apenasData = dataString.split('T')[0]; const [ano, mes, dia] = apenasData.split('-');
 return `${dia}/${mes}/${ano}`;
};
...
const KanbanCard = ({ item, onClick, colorBorder, showAuditButton, showReauditButton, isAuditing, onAudit }) => {
 ...
 {showAuditButton && (
 <button onClick={(e) => { e.stopPropagation(); onAudit(item.id); }}
 disabled={isAuditing}
 ...
 <button onClick={(e) => { e.stopPropagation(); onAudit(item.id); }}
 disabled={isAuditing}
 className={`p-1 rounded transition-colors flex items-center gap-1 font-bold ${isAuditing ? 'text-blue-600' : 'text-blue-600 hover:bg-blue-600'}`}
 >
 {isAuditing ? <FontAwesomeIcon icon={faSpinner} spin /> : <FontAwesomeIcon icon={faSync} />} <span className="hidden group-hover:inline">{isAuditing ? '...' : 'Re-auditar'}</span>
 </button>
 )}
...
export default function AuditoriaKanban({ filters }) {
 ...
 const handleStop = () => { pausedRef.current = true; setIsBulkProcessing(false); setAuditandoId(null); localStorage.removeItem('audit_queue'); localStorage.removeItem('audit_is_running'); };
 ...
 {/* COLUNA 3: Divergente */}
 <div className="flex flex-col bg-blue-600 rounded-xl border border-blue-600 h-full shadow-sm">
 <div className="p-3 border-b border-blue-600 bg-blue-600 rounded-t-xl flex justify-between items-center sticky top-0 z-10">
 <h3 className="font-bold text-blue-600 text-sm flex items-center gap-2"><FontAwesomeIcon icon={faExclamationTriangle} /> Divergente</h3>
 <span className="bg-white px-2 py-0.5 rounded text-xs font-bold text-blue-600 shadow-sm">{kanbanData.divergente.length}</span>
 </div>
 <div className="flex-1 overflow-y-auto p-2 scrollbar-thin">
