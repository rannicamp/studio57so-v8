...
export default function CategoriaFormModal({ isOpen, onClose, onSave, initialData, allCategories = [], defaultType = 'Despesa' }) {
 ...
 const availableParents = allCategories.filter(c => c.tipo === formData.tipo && (!isEditing || c.id !== formData.id)
 );

 // Filtra pela busca
 const filteredParents = availableParents.filter(c => (c.nome || '').toLowerCase().includes(searchParent.toLowerCase())
 );
 ...
 <div className="mt-1 w-full p-2 border rounded-md bg-white cursor-pointer flex justify-between items-center"
 ...
 <li className={`px-3 py-2 text-sm cursor-pointer hover:bg-gray-100 border-b border-gray-50 ${!formData.parent_id ? 'bg-blue-50 font-semibold text-blue-700' : 'text-gray-700'}`}
 onClick={() => {
 ...
 <li key={cat.id} className={`px-3 py-2 text-sm cursor-pointer hover:bg-blue-50 transition-colors ${String(formData.parent_id) === String(cat.id) ? 'bg-blue-50/50 font-semibold text-blue-700' : 'text-gray-700'}`}
 onClick={() => {
