...
export default function ConciliacaoManager({ contas }) {
 ...
 // Envia os dados para a janela de criação. // Armazenamos extratoItem.fitid numa coluna custom/temporária (via componente) ou aproveitamos os campos.
 ...
 {type === 'sistema' && isCartaoCredito && <FontAwesomeIcon icon={faCalendarCheck} className="flex-shrink-0 text-[10px] text-blue-600" title="Data da Transação (Cartão)" />}
 </div>
 ...
 {isCartaoCredito && <p className="text-xs text-blue-600 mt-2 flex items-center gap-1 font-semibold"><FontAwesomeIcon icon={faCreditCard} /> Modo Cartão: Usando Data da Transação para cruzamento</p>}
 </div>
 ...
 {calculadora ? (
 <div className={`flex items-center gap-6 px-6 py-3 rounded-xl border-2 shadow-sm ${calculadora.isMatch ? 'border-green-400 bg-green-50/80' : 'border-blue-600 bg-blue-600'}`}>
 ...
 {calculadora.isMatch ? (
 <button onClick={proceedWithMatch} className="ml-6 bg-blue-600 text-white to-emerald-600 text-white px-6 py-3 text-lg rounded-xl hover:text-white hover:to-emerald-700 font-extrabold flex items-center gap-2 animate-bounce shadow-[0_4px_15px_rgba(16,185,129,0.3)] transition-all">
 <FontAwesomeIcon icon={faLink} /> UNIR
 </button>
 ) : (
 <div className="ml-6 text-blue-600 text-xs font-bold max-w-[120px] text-center leading-tight bg-white p-2 border border-blue-600 rounded-lg shadow-sm">Diferença matemática identificada</div>
 )}
 ...
 <button onClick={handleConfirmMatches} disabled={isProcessing} className="bg-blue-600 from-indigo-600 text-white font-extrabold px-8 py-4 rounded-xl text-lg hover:from-indigo-700 hover: disabled:text-white disabled: flex items-center gap-3 shadow-[0_8px_25px_rgba(79,70,229,0.4)] transform transition hover:-translate-y-1 w-full md:w-auto justify-center">
