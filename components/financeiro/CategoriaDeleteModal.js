...
export default function CategoriaDeleteModal({ isOpen, onClose, onConfirm, categoria, allCategories }) {
 ...
 // (Lógica simples: remove a própria. Para remover filhas precisaria de lógica recursiva, // mas remover a própria já evita o erro circular básico).
 ...
 <input type="radio" name="deleteAction" value="orphan" checked={actionType === 'orphan'} onChange={() => setActionType('orphan')}
 className="mt-1 text-red-600 focus:ring-red-500"
 />
 <div className="ml-3">
 <span className="block font-medium text-gray-800 flex items-center gap-2">
 <FontAwesomeIcon icon={faEraser} className="text-gray-400" /> Remover categoria dos lançamentos
 </span>
 ...
 <input type="radio" name="deleteAction" value="migrate" checked={actionType === 'migrate'} onChange={() => setActionType('migrate')}
 ...
 {actionType === 'migrate' && (
 <select className="mt-3 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 border"
