...
export default function ExtratoUploader({ onFileLoaded }) {
 ...
 const [mode, setMode] = useState('upload'); const [isDragging, setIsDragging] = useState(false);
 ...
 if (data.csv) {
 onFileLoaded({ name: `${file.name} (Processado por IA)`, content: data.csv, extension: 'csv' });
 ...
 setTimeout(() => {
 setIsLoading(false);
 onFileLoaded({ name: "Texto Importado (Via Colar)", content: pastedText, extension: 'csv' });
 }, 500);
 };
 ...
 <div className="p-6">
 {mode === 'upload' && (
 <div className={`relative border-2 border-dashed rounded-xl p-10 text-center transition-all cursor-pointer group h-64 flex flex-col items-center justify-center ${isDragging ? 'border-blue-500 bg-blue-50 scale-[1.01]' : 'border-gray-300 hover:border-blue-400 hover:bg-gray-50'}`}
 onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}
 onClick={() => !isLoading && fileInputRef.current?.click()}
 >
 <input type="file" ref={fileInputRef} className="hidden" accept=".ofx,.csv,.pdf" onChange={(e) => e.target.files.length && processFile(e.target.files[0])}
 disabled={isLoading}
 />
 ...
 <div className="w-12 h-12 bg-blue-600 text-blue-600 rounded-full flex items-center justify-center">
