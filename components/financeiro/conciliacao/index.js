...
const STORAGE_KEY = 'conciliacao_state_v3'; // --- Helpers ---
...
const ListToolbar = ({ title, count, searchTerm, setSearchTerm, sortConfig, handleSort, dateFilter, handleDateFilter, color = 'gray', showConciliados, setShowConciliados, onAiMatch, isAiLoading }) => (
 ...
 {onAiMatch && (
 <button onClick={onAiMatch} disabled={isAiLoading}
 ...
 <input type="text" placeholder="Buscar descrição ou valor..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-1.5 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent text-xs"
 />
 </div>
...
export default function ConciliacaoManager({ contas }) {
 ...
 const [conciliationState, setConciliationState] = useState({ extrato: savedState.extrato || [], sistema: [], matches: savedState.matches || [], dateFilter: savedState.dateFilter || { startDate: '', endDate: '' } });
 ...
 lines.forEach((l, idx) => {
 if (l.toLowerCase().includes('valor') && l.toLowerCase().includes('descricao')) return; const separator = l.includes(';') ? ';' : ','; const parts = l.split(separator).map(x => x.trim());
 let d, v, desc = '';
 ...
 await supabase.from('lancamentos').update({ conciliado: true, status: 'Pago', data_pagamento: ex.data, id_transacao_externa: m.extratoId }).eq('id', m.sistemaId);
 }
 ...
 <ConciliacaoHeader contas={contas} selectedContaId={selectedContaId} setSelectedContaId={setSelectedContaId}
 ...
 <ListToolbar title="Sistema" count={processedLists.sistema.length} searchTerm={sysSearchTerm} setSearchTerm={setSysSearchTerm}
 ...
 <ConciliacaoItem key={item.id} item={item} type="sistema" listName="sistema" isCartaoCredito={isCartaoCredito} getDisplayDate={getDisplayDate}
 ...
 <div className="flex flex-col h-[750px] bg-white border border-gray-200 rounded-lg shadow-sm">
 <div className="p-2 border-b border-gray-100">
 <ListToolbar title="Extrato Bancário" count={processedLists.extrato.length} searchTerm={extSearchTerm} setSearchTerm={setExtSearchTerm}
 ...
 <ConciliacaoItem key={item.id} item={item} type="extrato" listName="extrato" isCartaoCredito={isCartaoCredito}
