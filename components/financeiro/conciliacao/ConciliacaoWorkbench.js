...
import { faLink, faPlus, faSearch, faSort, faSortUp, faSortDown, faPen, faCheckCircle, faMagic, faSpinner, faTimes, faCheckDouble } from '@fortawesome/free-solid-svg-icons';
...
const TransactionItem = ({ item, isSelected, onClick, type, onEdit, matchIndex, onRejectMatch, onHoverMatch, isHoveredMatch }) => {
 ...
 <button onClick={(e) => { e.stopPropagation(); onRejectMatch(); }}
...
export default function ConciliacaoWorkbench({ extratoItems = [], sistemaItems = [], onConciliar, onCriarLancamento, onEditarLancamento }) {
 ...
 setAiMatches(matches); toast.success(`IA: Encontrei ${matches.length} pares! Verifique e clique em 'Conciliar Todos' para confirmar.`);
 } else {
 ...
 return (
 <TransactionItem key={item.id} item={item} type="banco" isSelected={selectedBanco?.id === item.id}
 ...
 {aiMatches.length > 0 && (
 <button onClick={handleConciliarTodos} disabled={isAiLoading}
 ...
 <div className="p-3 bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
 <div className="flex justify-between items-center mb-2">
 <h3 className="font-bold text-gray-700 text-sm flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-blue-600"></span> Sistema</h3>
 <div className="flex gap-2 items-center">
 <button onClick={handleAiAutoMatch} disabled={isAiLoading} className={`text-[10px] px-3 py-1 rounded-full border font-bold transition-all flex items-center gap-1 shadow-sm ${isAiLoading ? 'bg-gray-100 text-gray-400' : 'bg-blue-600 text-white to-indigo-50 text-purple-700 border-purple-200 hover:scale-105'}`}>
 {isAiLoading ? <FontAwesomeIcon icon={faSpinner} spin /> : <FontAwesomeIcon icon={faMagic} />} {isAiLoading ? '...' : 'IA Match'}
 </button>
 <span className="text-[10px] bg-blue-600 text-blue-600 px-2 py-0.5 rounded-full font-bold">{processedSistema.length}</span>
 </div>
 </div>
 <div className="grid grid-cols-[80px_1fr_100px] gap-2 px-2">
 ...
 const matchData = aiMatches[matchIndex];
 return (
 <TransactionItem key={item.id} item={item} type="sistema" isSelected={selectedSistema?.id === item.id}
 matchIndex={matchIndex !== -1 ? matchIndex : null}
 isHoveredMatch={matchIndex !== -1 && hoveredMatchIndex === matchIndex}
 onHoverMatch={(isHovering) => setHoveredMatchIndex(isHovering ? matchIndex : null)}
 onRejectMatch={() => matchData && handleRejectAiMatch(matchData.extratoId, matchData.sistemaId)}
