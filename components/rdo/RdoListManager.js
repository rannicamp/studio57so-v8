...
export default function RdoListManager({ initialRdos, empreendimentosList, responsaveisList, isAdmin }) {
 ...
 <select onChange={(e) => setSelectedEmpreendimento(e.target.value)} value={selectedEmpreendimento} className="p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
 <option value="">Todos Empreendimentos</option>
 {empreendimentosList.map(emp => <option key={emp.id} value={emp.id}>{emp.nome}</option>)}
 </select>

 <select onChange={(e) => setSelectedResponsavel(e.target.value)} value={selectedResponsavel} className="p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
 ...
 <button onClick={() => setPeriod('week')} className="p-2 bg-gray-100 border border-gray-300 rounded-md text-sm w-full hover:bg-gray-200 transition-colors font-medium">Esta Semana</button>
 <button onClick={() => setPeriod('month')} className="p-2 bg-gray-100 border border-gray-300 rounded-md text-sm w-full hover:bg-gray-200 transition-colors font-medium">Este Mês</button>
 </div>
 <div>
 <label className="text-xs">Data Início</label>
 <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="p-2 border border-gray-300 rounded-md w-full focus:outline-none focus:ring-1 focus:ring-blue-500" />
 </div>
 <div>
 <label className="text-xs">Data Fim</label>
 <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="p-2 border border-gray-300 rounded-md w-full focus:outline-none focus:ring-1 focus:ring-blue-500" />
 </div>
 <input
 type="text"
 ...
 className="p-2 border border-gray-300 rounded-md w-full lg:col-span-4 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
 />
 </div>
 </div>
 )}
 ...
 {isAdmin && (
 <button onClick={(e) => handleDeleteRdo(e, rdo.id)} className="text-red-600 hover:text-red-800 font-medium transition-colors z-10 relative">Excluir</button>
 )}
 </td>
 </tr>
 ))
