...
const STATUS_CONFIG = {
 ...
 'aguardando material': { order: 2, colorClass: 'border-l-4 border-blue-600' },
...
export default function RdoForm({ initialRdoData, selectedEmpreendimento }) {
 ...
 className="font-medium text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 px-4 py-2 rounded-lg flex items-center gap-2 text-sm transition-colors"
 >
 ...
 <select name="condicoes_climaticas" value={rdoFormData.condicoes_climaticas || ''} onChange={handleRdoFormChange} onBlur={() => saveSectionData({ condicoes_climaticas: rdoFormData.condicoes_climaticas })} disabled={isRdoLocked} className="mt-1 block w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100">
 ...
 <input type="text" placeholder="Observação..." value={employee.observacao || ''} onChange={(e) => handleEmployeeChange(employee.id, 'observacao', e.target.value)} disabled={isRdoLocked} className="block w-full md:w-2/5 p-2 border border-gray-300 rounded-md text-sm disabled:bg-gray-100 focus:outline-none focus:ring-1 focus:ring-blue-500" />
 </li>
 ))}
 ...
 <select name="tipo" value={currentNewOccurrence.tipo} onChange={handleNewOccurrenceChange} className="flex-1 block w-full p-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500">
 {occurrenceTypes.map(type => (<option key={type} value={type}>{type}</option>))}
 </select>
 <textarea name="descricao" value={currentNewOccurrence.descricao} onChange={handleNewOccurrenceChange} rows="1" placeholder="Descreva a ocorrência..." className="flex-grow w-full md:w-1/2 block p-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"></textarea>
 <button type="button" onClick={handleAddOccurrence} disabled={!currentNewOccurrence.descricao.trim()} className="bg-green-600 text-white font-bold px-4 py-2 rounded-lg hover:bg-green-700 disabled:bg-gray-400 transition-colors">Adicionar</button>
 </div>
 )}
 <ul className="divide-y border rounded-md">
 ...
 */}
 ...
 2. Rascunho (Ctrl+P): Classes `print:...` entram em ação. - `print:block` e `print:static` garantem que apareça na folha.
