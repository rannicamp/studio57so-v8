...
function Avatar({ name, picUrl, size = 10 }) {
 ...
 return (
 <div className={`w-${size} h-${size} rounded-full bg-blue-600 text-white flex items-center justify-center text-white font-bold shrink-0`}
...
function ConversationItem({ conv, isSelected, onClick }) {
 ...
 {conv.unread_count > 0 && (
 <span className="absolute -top-1 -right-1 bg-blue-600 text-white text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1">
 {conv.unread_count}
 </span>
 )}
 </div>
...
function MessagePanel({ conv, organizacaoId, onBack, showProfile, onToggleProfile }) {
 ...
 ? 'bg-blue-600 text-white text-white rounded-tr-sm'
