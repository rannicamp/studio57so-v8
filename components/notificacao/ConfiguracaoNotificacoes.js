...
import { faBuilding, faCheck, faCheckCircle, faCog, faMobileAlt, faSave, faSpinner,
...
const ToggleSwitch = ({ label, active, onChange, icon, disabled = false }) => (
 <div className={`flex items-center justify-between py-2.5 px-3 border rounded-xl transition-all select-none min-w-[110px] sm:min-w-[130px] ${disabled ? 'opacity-50 cursor-not-allowed bg-gray-50' : 'cursor-pointer bg-white hover:border-blue-600 hover:shadow-sm'}`} onClick={() => !disabled && !!onChange && onChange()}
 >
 <div className="flex items-center gap-2">
 {icon && <FontAwesomeIcon icon={icon} className={`${active ? 'text-blue-600' : 'text-gray-400'}`} />}
 <span className={`text-xs font-bold ${active ? 'text-gray-900' : 'text-gray-500'}`}>{label}</span>
 </div>
 <div className={`w-10 h-5 rounded-full transition-colors relative flex items-center shrink-0 ml-3 ${active ? 'bg-blue-600' : 'bg-gray-200'}`}>
...
const NotificationRow = ({ template, catTabelas, conf, funcoes, onToggleActive, onTogglePush, onSelectRole }) => {
 ...
 return (
 <div className={`rounded-2xl transition-all border overflow-hidden ${isAtivo ? 'bg-white border-gray-200 hover:border-blue-600 shadow-sm' : 'bg-gray-50 border-gray-100 opacity-90'}`}>
 ...
 <div className={`w-12 h-12 flex shrink-0 items-center justify-center rounded-xl text-xl shadow-sm ${isAtivo ? 'bg-blue-600 text-white text-white' : 'bg-gray-200 text-gray-400'}`}>
 ...
 <ToggleSwitch label="Web" active={isAtivo} onChange={() => onToggleActive(template.id)} />
 <ToggleSwitch label="Mobile" active={pushAtivo && isAtivo} disabled={!isAtivo}
 ...
 <div className="px-3 py-1.5 border border-blue-600 bg-blue-600 text-blue-600 rounded-lg text-xs font-bold flex items-center gap-2 cursor-help" title="Configuração fixa: Autor da ação é sempre notificado.">
 ...
 return (
 <button key={funcao.id}
 onClick={() => onSelectRole(template.id, funcao.id)}
 className={`px-3 py-1.5 border rounded-lg text-xs font-bold transition-all
 ${selected ? 'bg-blue-600 text-white text-white border-blue-600 shadow-sm' : 'bg-white text-gray-600 border-gray-200 hover:border-blue-600 hover:bg-blue-600'}`}
 >
 {funcao.nome_funcao}
 </button>
 );
...
export default function ConfiguracaoNotificacoes() {
 ...
 <div className={`p-6 sm:p-8 rounded-3xl border ${isSubscribed ? 'border-green-200 bg-blue-600 text-white to-white' : 'border-blue-200 bg-blue-600 text-white to-white'} shadow-sm relative overflow-hidden`}>
 ...
 {isSubscribed ? "Sua conexão está segura. Você receberá notificações PUSH diretamente neste dispositivo mesmo com o app fechado." : "Ative a permissão para receber alertas em tempo real. Essencial para corretores e gerentes de equipe."}
 </p>
 </div>
 </div>
 ...
 <div className="pt-2">
 {!isSubscribed ? (
 <button onClick={handleSubscribeDevice} disabled={deviceLoading || (isIOS && !isStandalone)} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-500/20 text-white px-8 py-4 rounded-xl font-bold flex items-center justify-center gap-3 transition-all transform hover:-translate-y-0.5 active:translate-y-0"
 >
 ...
 <button onClick={handleUnsubscribeDevice} disabled={deviceLoading} className="text-red-500 text-sm font-bold bg-white border border-red-100 px-6 py-3 rounded-xl hover:bg-red-50 hover:text-red-600 transition-colors flex items-center gap-2"
 >
 ...
 <div>
 <h3 className="text-xl sm:text-2xl font-black text-gray-800 flex items-center gap-3">
 <div className="w-12 h-12 bg-blue-600 text-blue-600 flex items-center justify-center rounded-2xl">
 ...
 <div className="text-xs font-bold text-blue-600 uppercase tracking-widest bg-blue-600 px-3 py-1.5 rounded-lg border border-blue-600 shadow-sm">
 Conta Proprietária
 </div>
 </div>
 ...
 <FontAwesomeIcon icon={faSpinner} spin className="text-4xl mb-4 text-blue-600" />
 ...
 <NotificationRow key={template.id}
 template={template}
