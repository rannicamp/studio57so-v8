...
export default function RegraForm({ initialData, tabelas, campos, funcoes, variaveisVirtuais, onSubmit, isSaving, onCancel }) {
 ...
 <div className="bg-blue-600 text-white to-white p-6 rounded-3xl border border-blue-100 shadow-sm relative overflow-hidden group">
 ...
 {/* SESSÃO 2: AÇÃO */}
 <div className="bg-blue-600 text-white to-white p-6 rounded-3xl border border-blue-600 shadow-sm relative overflow-visible group">
 <div className="absolute top-0 right-0 w-32 h-32 bg-blue-600/50 rounded-bl-full -mr-10 -mt-10 transition-transform group-hover:scale-110"></div>

 <div className="relative z-10">
 <h4 className="text-[11px] font-extrabold text-blue-600 uppercase tracking-widest mb-5 flex items-center gap-2">
 <span className="w-5 h-5 rounded flex items-center justify-center bg-blue-600 text-blue-600 text-[10px]">3</span>
 Ação (O que Enviar)
 </h4>

 <div className="mb-6">
 <label className="block text-[10px] font-bold text-blue-600 mb-2 uppercase tracking-wide">Regras Especiais de Envio</label>
 <div className="flex flex-col md:flex-row gap-4">
 <div className="w-full md:w-1/2 flex flex-col gap-2">
 <label className="flex items-center gap-3 cursor-pointer bg-white p-4 rounded-xl border border-blue-600 hover:bg-blue-600 transition-colors h-full shadow-sm group-hover:border-blue-600">
 <input type="checkbox" name="enviar_para_dono" checked={formData.enviar_para_dono} onChange={handleChange} className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-600" />
 <div>
 <span className="text-xs font-bold text-gray-700 block transition-colors group-hover:text-blue-600">Responsável / Dono</span>
 <span className="text-[10px] text-gray-400">Notifica sempre quem criou o registro, independente do Cargo.</span>
 </div>
 </label>
 </div>
 <div className="w-full md:w-1/2 flex items-center bg-blue-600 p-4 rounded-xl border border-blue-600">
 <span className="text-xs text-blue-600 font-medium">Os Cargos (Funções) que receberão este alerta serão definidos pelos Donos de Franquia no próprio painel.</span>
 </div>
 </div>
 </div>

 <div className="space-y-4 relative bg-white/50 p-4 rounded-2xl border border-blue-600">
 {/* CAMPO TÍTULO */}
 <input required name="titulo_template" value={formData.titulo_template} onChange={handleChange} className="w-full px-4 py-3 border border-blue-600 rounded-xl text-sm font-bold placeholder-orange-300 focus:ring-2 focus:ring-blue-600 outline-none shadow-sm transition-all" placeholder="Título (Digite { para injetar variáveis)" autoComplete="off" />

 {/* CAMPO MENSAGEM */}
 <textarea required name="mensagem_template" value={formData.mensagem_template} onChange={handleChange} rows="3" className="w-full px-4 py-3 border border-blue-600 rounded-xl text-sm placeholder-orange-300 focus:ring-2 focus:ring-blue-600 outline-none shadow-sm transition-all resize-none" placeholder="Descrição ou mensagem completa... (Digite { para variáveis)" autoComplete="off" />
 ...
 <div className="mt-6 pt-5 border-t border-blue-600/50 flex flex-col md:flex-row justify-end items-center gap-4">
 <div className="w-full md:w-1/2 flex relative">
 <span className="absolute left-3 top-2.5 text-xs text-blue-600 font-bold">URL:</span>
 <input name="link_template" value={formData.link_template || ''} onChange={handleChange} className="w-full pl-12 pr-4 py-2.5 border border-blue-600 rounded-xl text-xs font-medium focus:ring-2 focus:ring-blue-600 outline-none shadow-sm placeholder-orange-200" placeholder="Ex: /projetos/123 (Ao clicar na notificação)" />
 </div>
 </div>
 </div>
 </div>
