...
export default function EmailActionMenu({ email, onAction, showCreateActivity = false, isOpen: controlledIsOpen, onToggle, accountId }) {
 ...
 let left = rect.left + window.scrollX - 224; if (left < 10) left = rect.left + window.scrollX;
 ...
 queryKey: ['emailFolders', finalAccountId], queryFn: async () => {
 if (!finalAccountId) return { folders: [] }; const res = await fetch(`/api/email/folders?accountId=${finalAccountId}`);
 ...
 const MenuContent = (
 <div className="fixed z-[99999] bg-white rounded-lg shadow-2xl border border-gray-100 w-56 animate-fade-in ring-1 ring-black/5 text-left"
 ...
 <FontAwesomeIcon icon={faFolderOpen} className="w-3 text-gray-400 group-hover:text-gray-600" /> Mover para...
 </div>
 ...
 <input autoFocus
 type="text" className="w-full text-xs border rounded px-2 py-1 outline-none focus:border-blue-500"
 placeholder="Nome..."
 value={newFolderName}
 onChange={(e) => setNewFolderName(e.target.value)}
 onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.key === 'Enter' && handleCreateSubmit(e)}
 />
 ...
 <button onClick={(e) => { e.stopPropagation(); setIsCreatingFolder(true); }}
 ...
 return (
 <button key={folder.path} onClick={(e) => { e.stopPropagation(); handleClick('move', folder.path); }}
 ...
 ${isInbox ? 'text-blue-500' : 'text-gray-400 group-hover:text-blue-500'} ${folder.level > 0 ? 'text-[10px]' : 'text-xs'}
 `} />
 ...
 <button onClick={() => handleClick('archive')} className="w-full text-left px-4 py-2 text-xs text-gray-700 hover:bg-blue-600 hover:text-blue-600 flex items-center gap-3">
