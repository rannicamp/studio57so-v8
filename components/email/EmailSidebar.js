...
const FolderContextMenu = ({ position, folder, onClose, onAction, isSystemFolder }) => {
 ...
 <FontAwesomeIcon icon={faEraser} className="text-blue-600 w-3" /> Esvaziar pasta
 </button>
 {!isSystemFolder && (
 <>
...
const AccountFolderTree = ({ account, selectedFolder, onSelectFolder, expandedPaths, toggleExpand, onCreateFolder }) => {
 ...
 // Hack seguro para Inbox sempre funcionar (só INBOX puro ou ENTRADA)
 const safePath = normalizeKey(path);
 if (safePath === 'INBOX' || safePath === 'ENTRADA') {
 newMap['INBOX'] = count;
 }
 });
 setCountsMap(newMap);
 ...
 onMutate: async ({ action, folderPath }) => {
 // EFEITO IMEDIATO NA TELA (OPTIMISTIC UPDATE)
 // Zera o contador na mesma hora que o usuário clica, sem esperar o servidor
 if (['markAllRead', 'empty', 'delete'].includes(action)) {
 setCountsMap(prev => {
 const newMap = { ...prev };
 newMap[folderPath] = 0;
 const safeFolderPath = normalizeKey(folderPath);
 ...
 <input type={showPwd ? 'text' : 'password'} className="w-full border border-gray-300 rounded px-2 py-1.5 pr-8 focus:outline-blue-500" value={newPassword}
 ...
 </div> <div className="flex gap-2 w-full mt-1">
 ...
 <button onClick={() => setIsReauthOpen(true)}
...
export default function EmailSidebar({
 ...
 const toastId = toast.loading('Aplicando regras em segundo plano...');
 try {
 // Fire and forget, não damos await para bloquear a UI
 fetch('/api/email/rules/apply', { method: 'POST' });
 setTimeout(() => toast.success('Regras rodando! O sistema organizará em background.', { id: toastId }), 1000);
 } catch (e) {
 toast.error('Erro ao solicitar regras', { id: toastId });
 }
 };
 ...
 <button onClick={handleApplyRules} title="Aplicar Regras" className="hover:text-purple-600 transition-colors p-1 rounded hover:bg-purple-100 text-purple-500"><FontAwesomeIcon icon={faWandMagicSparkles} /></button>
