...
export default function EmailViewPanel({ emailSummary, folder, onClose, onCreateRule }) {
 ...
 // --- LÓGICA DE MARCAR COMO LIDO (CORRIGIDA COM TRAVA DE TEMPO) ---
 ...
 // Zera o cronômetro para ESTE e-mail
 mountTimeRef.current = Date.now();

 return () => {
 // A mágica acontece aqui: ao fechar ou trocar de e-mail,
 // validamos quanto tempo passou desde a abertura.
 const timeSpent = Date.now() - mountTimeRef.current;
 const tempoMinimoLeitura = 1500; // 1.5 segundos

 if (currentEmailId && !isAlreadyRead && timeSpent > tempoMinimoLeitura) {
 // console.log("👀 Leitura válida (+1.5s). Marcando como lido:", currentEmailId);
 ...
 } else if (currentEmailId && !isAlreadyRead) {
 // console.log(`Ignorado: Leu por apenas ${timeSpent}ms (Mínimo: ${tempoMinimoLeitura}ms)`);
 }
 };
 ...
 if (!att || !att.filename || !emailSummary?.id || !folderIdentifier) {
 toast.error("Não foi possível baixar o anexo, dados incompletos.");
 return;
 }

 // Exibe um toast de carregamento rápido
 toast.info(`Iniciando download de ${att.filename}...`);

 // Chama a nova API via link dinâmico para forçar o download no navegador nativo
 let downloadUrl = `/api/email/attachment?uid=${emailSummary.id}&folder=${encodeURIComponent(folderIdentifier)}&filename=${encodeURIComponent(att.filename)}`;
 if (accountId) downloadUrl += `&accountId=${accountId}`;
