...
export default function EmailEditor({ value, onChange, placeholder }) {
 ...
 <MenuButton icon={faBold} action={() => editor.chain().focus().toggleBold().run()} isActive={editor.isActive('bold')} title="Negrito"
 />
 <MenuButton icon={faItalic} action={() => editor.chain().focus().toggleItalic().run()} isActive={editor.isActive('italic')} title="Itálico"
 />
 <div className="w-px h-6 bg-gray-300 mx-1" />
 <MenuButton icon={faListUl} action={() => editor.chain().focus().toggleBulletList().run()} isActive={editor.isActive('bulletList')} title="Lista"
 />
 <MenuButton icon={faListOl} action={() => editor.chain().focus().toggleOrderedList().run()} isActive={editor.isActive('orderedList')} title="Lista Numerada"
 />
 <MenuButton icon={faQuoteRight} action={() => editor.chain().focus().toggleBlockquote().run()} isActive={editor.isActive('blockquote')} title="Citação"
 />
 <div className="w-px h-6 bg-gray-300 mx-1" />

 {/* BOTÃO DE LINK — novo! */}
 <MenuButton icon={faLink} action={handleOpenLinkModal} isActive={hasLink}
 title={hasLink ? "Editar Link" : "Inserir Link"}
 />
 {hasLink && (
 <MenuButton icon={faUnlink} action={() => editor.chain().focus().extendMarkRange('link').unsetLink().run()}
 title="Remover Link"
 />
 )}

 <div className="w-px h-6 bg-gray-300 mx-1" />
 <MenuButton icon={faUndo} action={() => editor.chain().focus().undo().run()} title="Desfazer"
 />
 <MenuButton icon={faRedo} action={() => editor.chain().focus().redo().run()} title="Refazer"
 />
 </div>
