...
export default function EmailRulesConfig({ prefillData }) { const supabase = createClient();
 const { user } = useAuth();
 ...
 const [abortController, setAbortController] = useState(null); const [formData, setFormData] = useState({
 nome: '',
 ...
 const childrenMap = {}; const roots = []; const allPaths = new Set(folderList.map(f => f.path));
 ...
 setEditingId(null); setIsEditing(true); }
 ...
 method: 'POST',
 headers: { 'Content-Type': 'application/json' },
 body: JSON.stringify({ ruleId, cursor, limit: 50 }), signal: controller.signal
 });
 ...
 {isLoading ? <div className="text-center text-gray-400 py-4"><FontAwesomeIcon icon={faSpinner} spin /></div> : regras?.length === 0 ? (
 ...
 const contaNome = regra.email_configuracoes ? (regra.email_configuracoes.conta_apelido || regra.email_configuracoes.email) : 'Regra Antiga / Todas';
 ...
 {!regra.account_id && (
 <span className="text-[10px] text-blue-600 bg-blue-600 px-1 rounded border border-blue-600" title="Esta regra foi criada antes da atualização de multi-contas. Edite-a para vincular a uma conta específica.">
 ...
 <div className="bg-blue-600 p-4 rounded-lg border border-blue-600 space-y-3">
 <p className="text-xs font-bold text-blue-600 uppercase">ENTÃO (Ações)</p>
 ...
 <select value={acao.pasta} onChange={e => updateAction(idx, 'pasta', e.target.value)} className="w-full text-xs p-2 border rounded bg-white appearance-none"
 disabled={!formData.account_id || isLoadingFolders}
 >
 <option value="">
 {!formData.account_id ? 'Selecione uma conta acima primeiro' : isLoadingFolders ? 'Carregando pastas...' : 'Selecione a pasta...'}
 </option>
