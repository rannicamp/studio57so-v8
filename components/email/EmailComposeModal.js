...
export default function EmailComposeModal({ isOpen, onClose, initialData = null, onEmailSent }) {
    ...
    // Injeta Variáveis Dinâmicas
    const injectVariables = (text, userData) => {
        if (!text || !userData) return text;

        // Pega as propriedades unidas de AuthContext (Auth + tabela usuarios)
        let processedText = text;
        const variables = {
            '{{nome}}': userData.nome || userData.user_metadata?.name || '',
        ...
        Object.keys(variables).forEach(key => {
            // Regex global e case-insensitive para garantir que substitua todas as ocorrências
    ...
    const buildSignature = (config) => {
        ...
        let textoAssinatura = config.assinatura_texto;

        // Aplica a Injeção de Variáveis Dando o objeto User inteiro mapeado pelo AuthContext
        textoAssinatura = injectVariables(textoAssinatura, user);
    ...
    return createPortal(
        <div className="fixed inset-0 bg-black/70 z-[99999] flex items-center justify-center p-4 backdrop-blur-sm" style={{ pointerEvents: 'auto' }}>
            <div className="bg-white w-full max-w-4xl rounded-lg shadow-2xl flex flex-col h-[90vh] animate-fade-in-up">

                <div className="flex justify-between items-center px-6 py-4 border-b sticky top-0 bg-white rounded-t-lg z-20 shrink-0">
