...
export default function FuncionarioForm({ companies, empreendimentos, initialData, jornadas }) {
    ...
    const getInitialState = () => ({
        ...
        contract_role: '', admission_date: '', demission_date: '', status: 'Ativo',
        ...
        const promise = new Promise(async (resolve, reject) => {
            ...
            if (formData.cpf) {
                ...
                if (existingContact) {
                    ...
                    if (formData.email) await supabase.from('emails').insert({ contato_id: finalContatoId, email: formData.email, tipo: 'Principal' });
                    if (formData.phone) await supabase.from('telefones').insert({ contato_id: finalContatoId, telefone: formData.phone.replace(/\D/g, ''), tipo: 'Celular' });
                }
            }
