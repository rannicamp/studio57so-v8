...
export default function FuncionarioForm({ companies, empreendimentos }) {
  ...
  const handleSubmit = async (event) => {
    ...
    const uploadFile = async (file, docType) => {
      if (!file) return;
      const filePath = `${funcionarioId}/${docType}-${file.name.replace(/\s/g, '_')}`;
    ...
    try {
      ...
      ['photo', 'aso_doc', 'contract_doc', 'identity_doc'].forEach(id => {
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="lg:col-span-1">
              ...
              <div className="mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10 bg-gray-50">
                  <div className="text-center">
                      ...
                      <div className="mt-4 flex text-sm text-gray-600"><label htmlFor="photo" className="relative cursor-pointer rounded-md bg-white font-semibold text-blue-600 hover:text-blue-500"><span>Enviar arquivo</span><input id="photo" name="photo" type="file" className="sr-only" accept="image/*" onChange={handlePhotoChange} /></label><p className="pl-1">ou arraste</p></div>
        ...
        <div className="border-b border-gray-900/10 pb-8">
          ...
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-6 gap-6">
            <div className="sm:col-span-2"><label htmlFor="cep" className="block text-sm font-medium">CEP</label><IMaskInput mask="00000-000" id="cep" name="cep" value={formData.cep} unmask onAccept={(v) => handleMaskedChange('cep', v)} className="mt-1 w-full p-2 border rounded-md"/></div>
            <div className="sm:col-span-4"><label htmlFor="address_street" className="block text-sm font-medium">Rua / Logradouro</label><input type="text" id="address_street" name="address_street" value={formData.address_street || ''} onChange={handleChange} className="mt-1 w-full p-2 border rounded-md"/></div>
            <div className="sm:col-span-2"><label htmlFor="address_number" className="block text-sm font-medium">Número</label><input type="text" id="address_number" name="address_number" value={formData.address_number || ''} onChange={handleChange} className="mt-1 w-full p-2 border rounded-md"/></div>
            <div className="sm:col-span-4"><label htmlFor="address_complement" className="block text-sm font-medium">Complemento</label><input type="text" id="address_complement" name="address_complement" value={formData.address_complement || ''} onChange={handleChange} className="mt-1 w-full p-2 border rounded-md"/></div>
            <div className="sm:col-span-2"><label htmlFor="neighborhood" className="block text-sm font-medium">Bairro</label><input type="text" id="neighborhood" name="neighborhood" value={formData.neighborhood || ''} onChange={handleChange} className="mt-1 w-full p-2 border rounded-md"/></div>
            <div className="sm:col-span-2"><label htmlFor="city" className="block text-sm font-medium">Cidade</label><input type="text" id="city" name="city" value={formData.city || ''} onChange={handleChange} className="mt-1 w-full p-2 border rounded-md"/></div>
            <div className="sm:col-span-2"><label htmlFor="state" className="block text-sm font-medium">Estado</label><input type="text" id="state" name="state" value={formData.state || ''} onChange={handleChange} className="mt-1 w-full p-2 border rounded-md"/></div>
          </div>
        </div>
        ...
        <div className="border-b border-gray-900/10 pb-8">
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div><label htmlFor="aso_doc" className="block text-sm font-medium">Documento ASO</label><input id="aso_doc" name="aso_doc" type="file" onChange={handleFileChange(setAsoFile)} className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-blue-50 hover:file:bg-blue-100"/></div>
                <div><label htmlFor="contract_doc" className="block text-sm font-medium">Contrato</label><input id="contract_doc" name="contract_doc" type="file" onChange={handleFileChange(setContractFile)} className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-blue-50 hover:file:bg-blue-100"/></div>
                <div><label htmlFor="identity_doc" className="block text-sm font-medium">Doc. de Identidade</label><input id="identity_doc" name="identity_doc" type="file" onChange={handleFileChange(setIdentityFile)} className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-blue-50 hover:file:bg-blue-100"/></div>
            </div>
        </div>
