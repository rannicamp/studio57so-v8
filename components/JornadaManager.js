...
const JornadaForm = ({ jornada, onSave, onDelete, showToast }) => {
    ...
    const handleSave = async () => {
        ...
        const detalhesParaSalvar = formData.detalhes.map(({ id, ...rest }) => rest);
        await supabase.from('jornada_detalhes').upsert(detalhesParaSalvar, { onConflict: 'jornada_id, dia_semana' });
            ...
            {isExpanded && (
                <div className="p-4 space-y-6">
                    ...
                    <div className="overflow-x-auto">
                        <p className="text-md font-semibold mb-2">Configuração Semanal de Horários</p>
                        <table className="min-w-full">
                            <thead>
                                <tr>
                                    <th className="py-2 text-left">Dia</th>
                                    <th className="text-left">Entrada</th>
                                    <th className="text-left">Saída Almoço</th>
                                    <th className="text-left">Volta Almoço</th>
                                    <th className="text-left">Saída</th>
                                </tr>
                            </thead>
                            <tbody>
                                {/* Lógica corrigida para mostrar TODOS os 7 dias da semana */}
                                {formData.detalhes.sort((a,b) => a.dia_semana - b.dia_semana).map(d => (
                                    <tr key={d.dia_semana} className={(d.dia_semana === 0 || d.dia_semana === 6) ? 'bg-gray-100' : ''}>
                                        <td className="font-medium pr-4 py-1">{weekDays[d.dia_semana]}</td>
                                        <td><input type="time" value={d.horario_entrada || ''} onChange={e => handleDetailChange(d.dia_semana, 'horario_entrada', e.target.value)} className="p-1 border rounded w-full"/></td>
                                        <td><input type="time" value={d.horario_saida_intervalo || ''} onChange={e => handleDetailChange(d.dia_semana, 'horario_saida_intervalo', e.target.value)} className="p-1 border rounded w-full"/></td>
                                        <td><input type="time" value={d.horario_volta_intervalo || ''} onChange={e => handleDetailChange(d.dia_semana, 'horario_volta_intervalo', e.target.value)} className="p-1 border rounded w-full"/></td>
                                        <td><input type="time" value={d.horario_saida || ''} onChange={e => handleDetailChange(d.dia_semana, 'horario_saida', e.target.value)} className="p-1 border rounded w-full"/></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                        <p className="text-xs text-gray-500 mt-2">Dica: Deixe todos os campos de horário de um dia vazios para que ele seja considerado um dia de folga.</p>
                    </div>

                    <div className="flex justify-end gap-4 mt-4 border-t pt-4">
                        <button onClick={() => onDelete(formData.id)} className="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 font-semibold flex items-center gap-2"><FontAwesomeIcon icon={faTrash} /> Excluir</button>
                        <button onClick={handleSave} disabled={isSaving} className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 disabled:bg-gray-400 font-semibold flex items-center gap-2">
                            {isSaving ? <FontAwesomeIcon icon={faSpinner} spin /> : <FontAwesomeIcon icon={faSave} />} {isSaving ? 'Salvando...' : 'Salvar'}
                        </button>
                    </div>
                </div>
            )}
...
export default function JornadaManager({ initialJornadas }) {
    ...
    const handleAddNewJornada = async () => {
        ...
        const detalhes = weekDays.map((_, i) => ({ jornada_id: newJornada.id, dia_semana: i, horario_entrada: null, horario_saida: null, horario_saida_intervalo: null, horario_volta_intervalo: null }));
        ...
        showToast('Jornada criada! Expanda para configurar os horários.', 'success');
    };
