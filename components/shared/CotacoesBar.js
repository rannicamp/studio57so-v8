...
import { faChartLine, faDollarSign, faEuroSign, faTree, faGasPump, faBox, faLeaf, faDroplet, faWind, faFire, faSun, faShieldHalved,
...
export default function CotacoesBar({ visibleCotacoes }) {
 ...
 {c.name}: <strong className="ml-1.5 text-green-700 font-semibold">{c.type === 'currency' ? `R$ ${c.value}` : `$ ${c.value}`}</strong>
 </span>
 );
 })}
 </>
