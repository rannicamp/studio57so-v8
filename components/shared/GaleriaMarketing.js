...
export default function GaleriaMarketing({ anexos, storageBucket, onDelete, onToggleCorretor, isToggling }) {
 ...
 <button onClick={() => onToggleCorretor && onToggleCorretor(anexo)} disabled={isToggling} title={anexo.disponivel_corretor ? "Ocultar dos Corretores" : "Publicar para Corretores"} className={`h-8 w-8 flex flex-shrink-0 items-center justify-center rounded shadow transition-all mr-1 ${anexo.disponivel_corretor ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-gray-400 hover:bg-gray-500 text-white'}`}
 >
