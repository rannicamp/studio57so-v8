...
export default function FilePreviewModal({ anexo, onClose }) {
 ...
 return (
 <>
 <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] transition-opacity" onClick={onClose} />
 ...
 <a href={anexo.public_url + (anexo.public_url.includes('?') ? '&' : '?') + 'download=' + encodeURIComponent(anexo.nome_arquivo || 'documento')'} target="_blank" rel="noopener noreferrer" download={anexo.nome_arquivo} className="p-1.5 hover:bg-gray-700 rounded text-gray-400 hover:text-white transition-colors" title="Baixar">
 ...
 <img src={anexo.public_url} alt="Preview" className={`max-w-full max-h-full object-contain shadow-lg transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`} onLoad={() => setIsLoading(false)}
 />
 ) : isVideo ? (
 <video
 ...
 <a href={anexo.public_url + (anexo.public_url.includes('?') ? '&' : '?') + 'download=' + encodeURIComponent(anexo.nome_arquivo || 'documento')'} target="_blank" rel="noopener noreferrer" download className="bg-blue-600 text-white px-6 py-2 rounded shadow hover:bg-blue-700 transition">Baixar Arquivo ({ext})</a>
 </div>
 )}
 </div>
 </div>
