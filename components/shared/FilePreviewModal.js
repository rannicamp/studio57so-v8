...
export default function FilePreviewModal({ anexo, onClose }) {
 ...
 <a href={anexo.public_url + (anexo.public_url.includes('?') ? '&' : '?') + 'download=true'} target="_blank" rel="noopener noreferrer" download={anexo.nome_arquivo} className="p-1.5 hover:bg-gray-700 rounded text-gray-400 hover:text-white transition-colors" title="Baixar">
 ...
 <a href={anexo.public_url + (anexo.public_url.includes('?') ? '&' : '?') + 'download=true'} target="_blank" rel="noopener noreferrer" download className="bg-blue-600 text-white px-6 py-2 rounded shadow hover:bg-blue-700 transition">Baixar Arquivo ({ext})</a>
 </div>
 )}
 </div>
 </div>
