...
export default function GerenciadorAnexosGlobal({ anexos, viewMode: initialViewMode = 'grid', // 'list' or 'grid'
 storageBucket = 'empreendimento-anexos', onDelete, onToggleCorretor, isToggling,
 ...
 <input type="text" placeholder="Pesquisar anexos..." className="w-full pl-4 pr-10 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white shadow-sm"
 value={searchTerm}
 ...
 <button onClick={() => onToggleCorretor(anexo)} disabled={isToggling} title={anexo.disponivel_corretor ? "Ocultar dos Corretores" : "Publicar para Corretores"} className={`p-2 transition-colors flex items-center justify-center ${anexo.disponivel_corretor ? 'text-blue-600 hover:text-blue-800' : 'text-gray-400 hover:text-gray-600'}`}
 >
 ...
 <div className="flex-1 w-full bg-gray-100 relative overflow-hidden cursor-pointer"
 ...
 <div className="absolute inset-0 bg-blue-600 from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
