...
export default function FunilKanban({
 ...
 const walk = (x - startX); container.scrollLeft = scrollLeft - walk;
 };
 ...
 const handleDragOver = (e, colunaId) => { e.preventDefault(); if (draggedItem && draggedItem.type === 'card' && draggedItem.item.coluna_id !== colunaId) {
 ...
 const sortOptions = [
 { value: '', label: 'Padrão (Mais Recentes)' }, { value: 'nome_asc', label: 'Nome (A-Z)' }, { value: 'nome_desc', label: 'Nome (Z-A)' }, { value: 'created_at_asc', label: 'Entrada (Mais Antigo)' },
 ];
 ...
 let valA, valB;
 if (sortBy === 'nome') { valA = a.contatos?.nome || a.contatos?.razao_social || null; valB = b.contatos?.nome || b.contatos?.razao_social || null; } else if (sortBy === 'created_at') { valA = a[sortBy] ? new Date(a[sortBy]) : null; valB = b[sortBy] ? new Date(b[sortBy]) : null; }
 ...
 return (
 <div key={coluna.id} className={`w-80 flex-shrink-0 bg-white rounded-lg shadow-sm flex flex-col kanban-card transition-colors duration-200 ${dragOverColumnId === coluna.id ? 'bg-blue-50 border-2 border-blue-400' : ''}`} onDragOver={(e) => handleDragOver(e, coluna.id)} onDragLeave={handleDragLeave}
