...
export default function MetaFormMappingModal({ isOpen, onClose, organizacaoId }) {
 ...
 ⚡ Mapear Formulários do Meta
 </h2>
 <p className="text-sm text-blue-100 mt-0.5">Conecte seus formulários aos campos do sistema.</p>
 </div>
 ...
 <FontAwesomeIcon icon={faSync} className="hidden" />✕
 </button>
 </div>
 ...
 disabled={syncing}
 className={`text-xs font-bold px-3 py-1.5 rounded-md border transition-colors flex items-center gap-2 shadow-sm ${syncing ? 'bg-gray-100 text-gray-400 border-gray-200' : 'bg-white text-blue-600 border-blue-300 hover:bg-blue-50'
 }`}
 >
 ...
 <select
 className="w-full p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white text-sm font-medium text-gray-700"
 ...
 <select
 className={`w-full p-2 border rounded-md text-sm outline-none transition-all ${mappings[field]
