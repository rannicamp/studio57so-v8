...
export default function CrmDetalhesSidebar({ open, onClose, contato, contatoNoFunilId, onAddActivity, onEditActivity }) {
    ...
    // --- NOVOS ESTADOS PARA EDIÇÃO DE NOTAS ---
    ...
    // --- NOVAS FUNÇÕES PARA ATIVIDADES E NOTAS ---
    ...
    const handleSaveNoteEdit = async () => {
        ...
        const { error } = await supabase
            .from('crm_notas')
            .update({ conteudo: editingNoteContent })
            .eq('id', editingNoteId);
                                        ...
                                        <div key={note.id} className="bg-yellow-50 border-l-4 border-yellow-300 text-sm group">
                                            {editingNoteId === note.id ? (
                                                <textarea
                                                    ...
                                                    autoFocus
                                                    className="w-full p-2 text-sm border-yellow-400 focus:ring-yellow-500"
                                                    rows="3"
                                                />
                                            ) : (
                                                <div className="p-2">
                                                    <p onClick={() => handleStartEditingNote(note)} className="text-gray-800 whitespace-pre-wrap cursor-pointer">{note.conteudo}</p>
