...
import { useMuralPosts, useCreateMuralPost, useCreateMuralComment, useToggleMuralLike, useMuralAISuggestion, useDeleteMuralPost, useMuralAniversariantes } from './ChatMuralHooks';
...
export default function ChatMural() {
 ...
 const { data: aniversariantes = [] } = useMuralAniversariantes();
 ...
 className="px-3 py-2 bg-blue-600 text-white to-indigo-500 text-white rounded-lg font-semibold shadow-sm text-[12px] hover:shadow-md hover:text-white hover:to-indigo-600 disabled:opacity-50 transition-all flex items-center justify-center gap-1.5"
 ...
 {createPostMutation.isPending ? <FontAwesomeIcon icon={faSpinner} spin /> : 'Publicar'}
  ...
  {aniversariantes.length > 0 && (
    <div className="px-4 py-2.5 bg-gradient-to-r from-blue-50 to-indigo-50/50 border-b border-gray-100 shrink-0">
      ...
      <div className="flex gap-4 overflow-x-auto pb-1 scrollbar-none">
        {aniversariantes.map(aniv => {
          const nomeCurto = aniv.full_name.trim().split(' ')[0];
          const isHoje = aniv.dia === new Date().getDate();
          
          return (
            <div key={aniv.id} className="flex flex-col items-center gap-1 shrink-0 relative" title={`${aniv.full_name} - Dia ${aniv.dia}`}>
              <div className={`relative rounded-full p-[2px] ${isHoje ? 'bg-gradient-to-tr from-pink-500 via-red-500 to-yellow-500 animate-pulse' : 'bg-indigo-100 border border-indigo-200'}`}>
                {aniv.foto_url ? (
                  <img src={aniv.foto_url} className="w-8 h-8 rounded-full object-cover border border-white" alt={aniv.full_name} />
                ) : (
                  <div className="w-8 h-8 rounded-full bg-indigo-600 text-white flex items-center justify-center text-[10px] font-black border border-white uppercase">
                    {nomeCurto.charAt(0)}
                  </div>
                )}
                <span className="absolute -bottom-1 -right-1 bg-white text-[8px] font-black text-indigo-600 border border-indigo-100 rounded-full px-1.5 py-0.2 shadow-sm leading-none flex items-center justify-center min-w-[14px] h-[14px]">
                  {aniv.dia}
                </span>
              </div>
              <span className={`text-[10px] font-bold truncate max-w-[55px] ${isHoje ? 'text-pink-600 font-extrabold' : 'text-gray-500'}`}>
                {isHoje ? 'Hoje! 🎉' : nomeCurto}
              </span>
            </div>
          );
        })}
