        ...
        if (isOpen) {
            if (isEditing) {
                setItem({
                    ...
                    fornecedor_nome: itemToEdit.fornecedor?.nome || '',
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-3xl">
                ...
                <div className="space-y-4">
                    <div className="relative">
                        ...
                        {isItemSelected ? (
                            ...
                            <>
                                <input type="text" value={searchTerm} onChange={handleSearchChange} placeholder="Digite para buscar ou descrever..." className="mt-1 w-full p-2 border rounded-md" autoComplete="off" />
                ...
                <div className="flex justify-end gap-4 pt-6 mt-4 border-t">
                    ...
                    <button onClick={handleSaveClick} disabled={isSaving || !isItemSelected} className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed"> {isSaving ? <FontAwesomeIcon icon={faSpinner} spin /> : (isEditing ? 'Salvar Alterações' : 'Salvar Item')} </button>
                </div>
            </div>
        </div>
    );
