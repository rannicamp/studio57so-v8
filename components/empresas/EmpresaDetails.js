...
import {
  ...
  faSpinner, faTrash, faEye, faRightLeft, faEdit, faTimes,
  faFileContract, faTableCellsLarge, faBars, faHome, faUserTie,
  faEnvelope, faPhone, faIdCard, faMapMarkerAlt, faFolderOpen
...
const EMPRESA_UI_STATE_KEY = 'STUDIO57_EMPRESA_UI_V2';
...
function InfoField({ label, value, fullWidth = false }) {
  ...
  return (
    <div className={`${fullWidth ? "md:col-span-3" : ""} space-y-1`}>
      <p className="text-[10px] font-black text-gray-400 uppercase tracking-wider">{label}</p>
      <p className="text-sm font-semibold text-gray-800 whitespace-pre-wrap leading-relaxed">{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm flex flex-col justify-between gap-3 hover:border-black transition-all cursor-pointer">
      <div className="flex items-center justify-between">
        <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wider truncate pr-2">{title}</p>
        {icon && (
          <div className="w-8 h-8 rounded-lg bg-blue-50 text-black flex items-center justify-center flex-shrink-0">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-2xl">
        <h3 className="text-lg font-bold mb-4 text-gray-800 flex items-center gap-2">
          <FontAwesomeIcon icon={faRightLeft} className="text-black" /> Mover Arquivo
        </h3>

        <div className="mb-4">
          <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Nova Categoria/Aba</label>
          <select value={categoria} onChange={e => setCategoria(e.target.value)} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-1 focus:ring-black text-sm font-semibold text-gray-700">
        ...
        <div className="mb-6">
          <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Realinhar Tipo do Documento</label>
          <select value={tipoId} onChange={e => setTipoId(e.target.value)} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-1 focus:ring-black text-sm font-semibold text-gray-700">
        ...
        <div className="flex justify-end gap-3 mt-8">
          <button onClick={onClose} disabled={isSaving} className="px-4 py-2 border rounded-lg text-xs font-bold text-gray-600 hover:bg-gray-100 transition-colors">Cancelar</button>
          <button onClick={handleSave} disabled={isSaving} className="px-4 py-2 bg-black hover:bg-gray-900 text-white rounded-lg text-xs font-bold transition-colors flex items-center justify-center min-w-[80px]">
...
const ModalEditarAnexo = ({ isOpen, onClose, anexoInfo, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-2xl">
        <h3 className="text-lg font-bold mb-4 text-gray-800 flex items-center gap-2">
          <FontAwesomeIcon icon={faEdit} className="text-black" /> Editar Arquivo
        </h3>
        <div className="mb-4">
          <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Nome do Arquivo</label>
          <input type="text" value={nome} onChange={e => setNome(e.target.value)} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-1 focus:ring-black text-sm font-semibold text-gray-700" />
        </div>
        <div className="mb-6">
          <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Descrição</label>
          <textarea value={descricao} onChange={e => setDescricao(e.target.value)} className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-lg outline-none focus:ring-1 focus:ring-black text-sm font-semibold text-gray-700 min-h-[80px]" />
        </div>
        <div className="flex justify-end gap-3 mt-8">
          <button onClick={onClose} disabled={isSaving} className="px-4 py-2 border rounded-lg text-xs font-bold text-gray-600 hover:bg-gray-100 transition-colors">Cancelar</button>
          <button onClick={handleSave} disabled={isSaving} className="px-4 py-2 bg-black hover:bg-gray-900 text-white rounded-lg text-xs font-bold transition-colors flex items-center justify-center min-w-[120px]">
...
export default function EmpresaDetails({ empresa, initialAnexos, documentoTipos, organizacaoId }) {
  ...
  const handleMoveSave = (updatedAnexo) => {
    setAnexos(currentAnexos => currentAnexos.map(a => a.id === updatedAnexo.id ? { ...a, categoria_aba: updatedAnexo.categoria_aba, tipo_documento_id: updatedAnexo.tipo_documento_id, tipo: documentoTipos.find(t => t.id === updatedAnexo.tipo_documento_id) } : a
    ));
  };

  const handleEditSave = (updatedAnexo) => {
    setAnexos(currentAnexos => currentAnexos.map(a => a.id === updatedAnexo.id ? { ...a, nome_arquivo: updatedAnexo.nome_arquivo, descricao: updatedAnexo.descricao } : a
    ));
  };
  ...
  const TabButton = ({ tabId, label }) => (
    <button 
      ...
      className={`whitespace-nowrap py-3 px-1 border-b-2 text-sm transition-all font-semibold ${
        activeTab === tabId 
  ...
  return (
    <div className="p-6 bg-white rounded-lg h-full space-y-8">
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b pb-6">
        <div className="flex items-center gap-4 flex-1">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-black text-gray-900 break-words tracking-tight uppercase leading-snug">
              {empresa.nome_fantasia || empresa.razao_social}
            </h1>
            <p className="text-xs text-gray-500 font-bold uppercase mt-1">CNPJ: {empresa.cnpj || 'Não informado'}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <button 
            onClick={() => window.dispatchEvent(new CustomEvent('edit-empresa', { detail: empresa }))} 
            className="bg-black hover:bg-gray-900 text-white font-bold py-2.5 px-4 rounded-lg shadow-sm text-xs uppercase tracking-wider transition-all flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard title="Total Documentos" value={kpiData.totalAnexos} icon={faFileLines} />
        <KpiCard title="Contábil" value={kpiData.anexosContabeis} icon={faBoxOpen} />
        <KpiCard title="Jurídicos" value={kpiData.anexosJuridicos} icon={faBuilding} />
        <KpiCard title="Marketing" value={kpiData.anexosMarketing} icon={faUpload} />
      </div>
      ...
      <div className="pb-10">
        {activeTab === 'ficha' && (
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="bg-white p-6 border border-gray-200 rounded-lg shadow-sm relative overflow-hidden space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-2">
                ...
                <InfoField label="Capital Social" value={empresa.capital_social ? `R$ ${new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2 }).format(empresa.capital_social)}` : ''} />
                <InfoField label="Natureza Jurídica" value={empresa.natureza_juridica} />
                <InfoField label="ID Meta Business" value={empresa.meta_business_id} />
                <InfoField label="Objeto Social" value={empresa.objeto_social} fullWidth={true} />
              </div>
            </div>
            ...
            {/* Bloco 3: Responsável Legal */}
            <div className="bg-white p-6 border border-gray-200 rounded-lg shadow-sm relative overflow-hidden space-y-4">
              <div className="absolute top-0 left-0 w-1 h-full bg-black"></div>
              <h3 className="text-xs font-bold text-gray-500 uppercase tracking-widest flex items-center gap-2">
                <FontAwesomeIcon icon={faUserTie} className="text-black" />
        ...
        {['contabil', 'juridico', 'marketing'].includes(activeTab) && (
          <div className="space-y-6 animate-fade-in mt-6">
            ...
            <GerenciadorAnexosGlobal anexos={anexos.filter(a => a.categoria_aba === activeTab)} viewMode={viewMode} storageBucket="empresa-anexos"
              onDelete={handleDeleteAnexo} onPreview={setPreviewAnexo} onMove={setMoveAnexoTarget} onEdit={setEditAnexoTarget} />
          </div>
        )}
      </div>
