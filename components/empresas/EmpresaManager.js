      ...
      <div className="w-1/3 min-w-[320px] max-w-[400px] border-r border-gray-200 bg-white flex flex-col h-full shadow-sm z-10">
        ...
        <div className="p-5 border-b border-gray-100 space-y-4 shrink-0 bg-white">
          <div className="flex justify-between items-center">
            <h2 className="text-lg font-bold text-gray-900 uppercase tracking-wider">Empresas</h2>
            {canCreate && (
              <button
                onClick={handleOpenCreate}
                className="bg-black hover:bg-gray-900 text-white font-bold py-2 px-3 rounded-lg shadow-sm text-[10px] uppercase tracking-wider transition-all flex items-center gap-2 active:scale-95"
              >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 block w-full bg-white border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-semibold text-gray-700 focus:outline-none focus:ring-1 focus:ring-black focus:border-black transition-all shadow-sm placeholder-gray-400"
            />
          </div>
        </div>
                ...
                return (
                  <li key={empresa.id}>
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-lg transition-all duration-200 group flex gap-3 items-center border
                      ${isSelected
                        ? 'bg-gray-100 border-gray-200 shadow-sm font-semibold'
                    ...
                    >
                      ...
                      ${isSelected ? 'shadow-inner' : 'group-hover:shadow-sm'}`} style={{ backgroundImage: empresa.logo_url ? `url(${empresa.logo_url})` : empresa.imagem_url ? `url(${empresa.imagem_url})` : 'none', backgroundColor: !(empresa.logo_url || empresa.imagem_url) ? (isSelected ? '#000000' : '#F3F4F6') : undefined, color: !(empresa.logo_url || empresa.imagem_url) ? (isSelected ? 'white' : '#6B7280') : undefined }}>
                        {!(empresa.logo_url || empresa.imagem_url) && initials}
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className={`text-sm font-bold truncate ${isSelected ? 'text-black' : 'text-gray-900'}`}>
      ...
      <div className="flex-1 bg-white overflow-y-auto custom-scrollbar relative">
        {!selectedEmpresaId ? (
          <div className="h-full flex flex-col items-center justify-center text-gray-400 p-8 space-y-4">
            ...
            <h3 className="text-lg font-bold text-gray-800">Nenhuma empresa selecionada</h3>
            <p className="text-xs max-w-sm text-center text-gray-500 font-medium">
