...
export default function ActivityBasicInfo({ formData, setFormData, organizacaoId }) {
 ...
 <input type="text" name="nome" value={formData.nome || ''} onChange={handleChange} required placeholder="Ex: Concretagem da Laje L1"
 ...
 <textarea name="descricao" value={formData.descricao || ''} onChange={handleChange} rows="3" placeholder="Detalhes adicionais..."
 ...
 placeholder="Digite para buscar a atividade principal..." className="w-full p-2.5 pl-9 pr-8 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
 />
 {searchTerm && (
 <button type="button" onClick={handleClearParent} className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-red-500 transition-colors"
 >
 ...
 <li key={activity.id} onMouseDown={() => handleSelectParent(activity)}
