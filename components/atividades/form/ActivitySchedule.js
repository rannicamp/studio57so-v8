...
export default function ActivitySchedule({ formData, setFormData }) {
 ...
 if (type === 'atividade') {
 setFormData(prev => ({ ...prev, duracao_horas: null, hora_inicio: null, duracao_dias: 1,
 tipo_atividade: 'Tarefa'
 }));
 } else {
 setFormData(prev => ({ ...prev, duracao_horas: 1, hora_inicio: '09:00', duracao_dias: 0,
 ...
 <button type="button" onClick={() => handleTypeToggle('atividade')} className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${!isEvent ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
 >
 Atividade (Dias)
 </button>
 <button type="button" onClick={() => handleTypeToggle('evento')} className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${isEvent ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
 >
 Evento (Horas)
 </button>
 </div>
 ...
 <input type="date" name="data_inicio_prevista" value={formData.data_inicio_prevista || ''} onChange={handleChange} className="w-full p-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
 />
 </div>
 <div>
 <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Duração (Dias Úteis)</label>
 <input type="number" name="duracao_dias" min="0.5" step="0.5" value={formData.duracao_dias} onChange={handleChange} onBlur={handleBlur}
 ...
 {serverEndDate ? new Date(serverEndDate.replace(/-/g, '/')).toLocaleDateString('pt-BR') : '--/--/----'
 }
 ...
 <input type="date" name="data_inicio_prevista" value={formData.data_inicio_prevista || ''} onChange={handleChange} className="w-full p-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 outline-none"
 />
 </div>
 <div>
 <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Hora Início</label>
 <input type="time" name="hora_inicio" value={formData.hora_inicio || ''} onChange={handleChange} className="w-full p-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 outline-none"
 />
 </div>
 <div className="md:col-span-2">
 <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Duração (Horas)</label>
 <input type="number" name="duracao_horas" min="0.5" step="0.5" value={formData.duracao_horas || ''} onChange={handleChange} onBlur={handleBlur}
 placeholder="Ex: 1.5 para 1h30"
 className="w-full p-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 outline-none"
 />
 </div>
 </>
 )}
 ...
 <input type="checkbox" id="is_recorrente" name="is_recorrente" checked={formData.is_recorrente} onChange={handleChange} className="h-4 w-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
 ...
 <input type="number" name="recorrencia_intervalo" value={formData.recorrencia_intervalo} onChange={handleChange} onBlur={(e) => {
 if(!e.target.value || e.target.value < 1) setFormData(prev => ({...prev, recorrencia_intervalo: 1}))
 }}
 min="1" className="w-16 p-2 text-sm border border-blue-200 rounded-lg text-center"
 />
 <select name="recorrencia_tipo" value={formData.recorrencia_tipo} onChange={handleChange} className="flex-1 p-2 text-sm border border-blue-200 rounded-lg bg-white"
 >
 ...
 <input type="date" name="recorrencia_fim" value={formData.recorrencia_fim || ''} onChange={handleChange} className="w-full p-2 text-sm border border-blue-200 rounded-lg"
 />
 </div>
 </div>
 )}
