...
export default function ActivityCopilot({ isOpen, onClose, organizacaoId, usuarioId, onSuccess }) {
 ...
 {/* Botão de Áudio */}
 <button type="button" onClick={handleAudioRecord} disabled={loadingChat}
 className={`p-3 rounded-full transition-colors flex-shrink-0 shadow-sm ${
 isRecording ? 'bg-red-500 text-white animate-pulse shadow-red-200' : 'bg-white border border-gray-200 text-gray-600 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200'
 }`}
 ...
 <button onClick={() => handleSaveSingle(i, activity)} disabled={saving || isSaved} className={`text-xs font-bold px-3 py-1.5 rounded-md flex items-center gap-2 transition-colors ${isSaved ? 'bg-green-100 text-green-700 cursor-default' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-sm'}`}
 >
