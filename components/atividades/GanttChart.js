...
import { faSearchPlus, faSearchMinus, faCalendarDay, faChevronDown, faChevronRight, faCheckSquare, faSquare } from '@fortawesome/free-solid-svg-icons';
...
const ROW_HEIGHT = 32; const HEADER_HEIGHT = 48; // Componente da Coluna de Dia
const DayColumn = ({ date, width }) => {
 const day = date.getUTCDay();
 const isWeekend = day === 6 || day === 0;
 return (
 <div className={`text-center border-r border-gray-100 flex-shrink-0 flex flex-col justify-center ${isWeekend ? 'bg-red-50/50' : ''}`}
...
export default function GanttChart({ activities, onEditActivity }) {
 ...
 flatList.push({ ...node, depth, hasChildren: node.subTasks.length > 0,
 ancestors: parentIds
 });
 ...
 {selectedStatuses.length === 0 || selectedStatuses.length === STATUS_OPTIONS.length ? `Todos (${activities?.length || 0})` : `${selectedStatuses.length} selecionados`}
 </span>
 ...
 {/* Header Fixo */}
 <div className="border-b border-gray-200 bg-gray-50 flex items-center px-3 font-bold text-gray-600 text-[10px] uppercase tracking-wider shrink-0"
 ...
 <div key={task.id} className={`border-b border-gray-50 flex items-center pr-3 hover:bg-blue-50 cursor-pointer transition-colors ${!task.atividade_pai_id ? 'bg-gray-50/50' : ''}`}
 ...
 {task.hasChildren && (
 <button onClick={(e) => toggleCollapse(task.id, e)}
 ...
 <DayColumn key={date.toISOString()} date={date} width={columnWidth} />
 ))}
 </div>
 {/* Corpo do Gráfico */}
 ...
 {/* Linhas de Tarefa */}
 {visibleTasks.map(task => (
 <div key={task.id} className="relative border-b border-gray-50 hover:bg-gray-50 transition-colors group"
 ...
 {/* Marcador de Hoje */}
 {todayMarkerPosition && (
 <div className="absolute top-0 bottom-0 w-[1px] bg-red-500 z-30 pointer-events-none shadow-[0_0_8px_rgba(239,68,68,0.6)]" style={{ left: `${todayMarkerPosition}px` }} >
