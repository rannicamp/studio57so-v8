...
export default function AtividadeFiltros({ filters, onChange, onClear, listas }) {
 ...
 <select value={filters.empresa || ''} onChange={e => onChange('empresa', e.target.value)} className="w-full p-2 border rounded-md text-sm bg-gray-50 focus:bg-white"
 >
 ...
 <select value={filters.empreendimento || ''} onChange={e => onChange('empreendimento', e.target.value)} className="w-full p-2 border rounded-md text-sm bg-gray-50 focus:bg-white"
 ...
 selectedIds={filters.status} onChange={(selected) => onChange('status', selected)}
