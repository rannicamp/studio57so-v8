...
export default function SimuladorFinanceiroPublico({ empreendimentos }) {
 ...
 <input type="text" placeholder="Nome do Cliente" value={cliente.nome} onChange={(e) => {
 ...
 {selectedEmpreendimentoId && empreendimentos.find(e => e.id == selectedEmpreendimentoId)?.observacoes && (
 ...
 Condições Comerciais do Empreendimento
 </h5>
 <p className="text-xs text-blue-900 whitespace-pre-wrap leading-relaxed">
 {empreendimentos.find(e => e.id == selectedEmpreendimentoId)?.observacoes}
 </p>
 </div>
 )}
