...
import { faPlus, faTrash, faSearch, faFilter, faTimes, faMagic, faEraser, faSpinner, faCheckDouble
...
export default function BimFilterPanel({ viewer, projetoBimId }) {
 ...
 viewer.impl.visibilityManager.aggregateIsolate([]); viewer.showAll();
 viewer.clearSelection(); const allModels = viewer.impl.modelQueue().getModels();
 ...
 {filters.length > 1 && (
 <button onClick={() => removeRow(filter.id)} className="text-gray-300 hover:text-red-500 p-1 transition-colors"
 title="Remover regra"
 >
