...
import { faSpinner, faExclamationCircle, faCamera, faClock, faMapMarkerAlt, faComment, faPaperPlane, faTrash } from '@fortawesome/free-solid-svg-icons';
...
const STATUS_CONFIG = {
 'aberta': { label: 'Aberta', color: 'bg-red-50 text-red-600 border-red-100 hover:bg-red-100', next: 'em_analise' },
 'em_analise': { label: 'Em Análise', color: 'bg-blue-600 text-blue-600 border-blue-600 hover:bg-blue-600', next: 'concluida' },
 'concluida': { label: 'Concluída', color: 'bg-green-50 text-green-600 border-green-100 hover:bg-green-100', next: 'aberta' }
};
 ...
 *,
 ...
 <div className="w-full h-full bg-blue-600 text-white text-white flex items-center justify-center text-xs font-bold">{initials}</div>
 )}
 </div>
 ...
 <button onClick={(e) => { e.stopPropagation(); changeStatus({ notaId: note.id, currentStatus: note.status }); }}
 ...
 <button onClick={(e) => { e.stopPropagation(); softDeleteNote(note.id); }}
 ...
 <button onClick={(e) => { e.stopPropagation(); setOpenChatId(isChatOpen ? null : note.id); }} className={`text-[10px] font-bold flex items-center gap-1 transition-colors ${isChatOpen ? 'text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
 >
 <FontAwesomeIcon icon={faComment} /> {commentCount > 0 ? `${commentCount} Respostas` : 'Responder'}
 </button>
 </div>
 ...
 placeholder="Escreva uma resposta..." className="flex-1 text-xs px-3 py-2 rounded-full border border-gray-300 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-100"
 autoFocus
 />
