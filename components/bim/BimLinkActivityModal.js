...
export default function BimLinkActivityModal({ isOpen, onClose, activities = [], // Recebe a lista de atividades do page.js
 ...
 const filteredActivities = activities.filter(act => act.nome.toLowerCase().includes(searchTerm.toLowerCase())
 );
 ...
 {selectedCount > 1 ? `Vinculando ${selectedCount} elementos selecionados` : `Elemento: ${targetElement?.elementName || targetElement?.externalId || '...'}`
 }
 </p>
 </div>
 ...
 <input type="text" placeholder="Buscar atividade..." className="w-full pl-8 pr-3 py-2 text-xs border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
 value={searchTerm}
 ...
 selectedActivityId === act.id ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-500' : 'border-gray-200 hover:border-blue-300 hover:bg-gray-50'
 }`}
 >
