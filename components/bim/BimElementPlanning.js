...
import { faPlus, faLink, faUnlink, faCalendarAlt, faTasks, faSpinner
} from '@fortawesome/free-solid-svg-icons';
import { toast } from 'sonner';

export default function BimElementPlanning({ elementExternalId, projetoBimId, elementName,
 onOpenLink, onOpenCreate }) {
 ...
 .select(`
 id,
 atividade:activities ( id, nome, status, data_inicio_prevista, data_fim_prevista
 )
 `)
 ...
 <button onClick={() => { if(confirm("Desvincular atividade?")) desvincular(v.id) }}
 ...
 <span>
 {act.data_inicio_prevista ? new Date(act.data_inicio_prevista).toLocaleDateString('pt-BR') : 'S/D'} {' -> '} {act.data_fim_prevista ? new Date(act.data_fim_prevista).toLocaleDateString('pt-BR') : 'S/D'}
 </span>
 </div>
 </div>
 </div>
