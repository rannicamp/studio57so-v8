...
import { useAuth } from '../../contexts/AuthContext'; import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
...
export default function BimUploadModal({ isOpen, onClose, preSelectedContext, onSuccess,
 ...
 <select value={selectedEmpresa} onChange={(e) => {setSelectedEmpresa(e.target.value); setSelectedObra('');}} className="w-full border border-gray-300 rounded-lg p-2 text-sm bg-white outline-none"
 ...
 <select value={selectedObra} onChange={(e) => setSelectedObra(e.target.value)} disabled={!selectedEmpresa || isUploading} className="w-full border border-gray-300 rounded-lg p-2 text-sm disabled:bg-gray-100 bg-white outline-none"
 >
 <option value="">Selecione...</option>
 ...
 <select value={selectedDisciplina} onChange={(e) => setSelectedDisciplina(e.target.value)} disabled={isUploading}
 ...
 <button onClick={handleDirectUpload} disabled={isUploading || !file || (!isVersionMode && (!selectedDisciplina || !selectedObra))}
