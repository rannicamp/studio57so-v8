...
import { faCheckSquare, faSquare, faClock, faEllipsisV, faCloudUploadAlt, faPen, faDatabase, faTrash,
...
const BimFileItem = React.memo(({ file, isActive, isSelected, onFileSelect, onToggleModel, onAction }) => {
 ...
 return (
 <div onClick={(e) => { if (isMenuOpen) return; // Se o menu tá aberto, não seleciona o arquivo
 e.stopPropagation(); if(!isProcessing && !isError) onFileSelect(file); }} className={`group relative p-2 rounded-lg border cursor-pointer transition-all mb-1 ${isActive ? 'bg-blue-600 border-blue-600 text-white shadow-md' : 'bg-white border-gray-100 hover:border-blue-200 hover:shadow-sm'} ${isProcessing ? 'opacity-70 cursor-wait' : 'hover:scale-[1.01]'}
 ...
 <button onClick={(e) => { e.stopPropagation(); setIsMenuOpen(!isMenuOpen); }} className={`p-1 px-2 rounded hover:bg-black/10 transition-all z-[100] ${isActive ? 'text-white' : 'text-gray-400 hover:text-gray-600'}`}
 >
 ...
 {isMenuOpen && (
 <div className="absolute right-0 top-7 w-48 bg-white border border-gray-200 shadow-2xl rounded-xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 origin-top-right"
 ...
 <button onClick={(e) => handleMenuAction(e, 'sync')} className="w-full text-left px-4 py-2.5 text-[11px] font-black text-blue-700 bg-blue-50/50 hover:bg-blue-600 hover:text-white flex items-center gap-3 transition-all border-y border-blue-100"
 >
