...
function AutodeskViewerAPI({ urn, onViewerReady, onSelectionChange }) { const viewerContainerRef = useRef(null);
 ...
 const options = {
 env: 'AutodeskProduction', api: 'derivativeV2', getAccessToken: async (cb) => {
 try {
 ...
 // Não damos finish() para não quebrar a navegação rápida, // mas limpamos a ref se o componente for destruído
 }
 };
 ...
 }, [urn, isInitialized]); // 4. Redimensionamento
 useEffect(() => {
