...
import { faLayerGroup, faTasks, faCamera, faStickyNote, faFilter, faListCheck, faChevronDown, faChevronRight } from '@fortawesome/free-solid-svg-icons';
...
export default function BimInspector({ elementExternalId, selectedElements = [], selectedCount = 0, projetoBimId, urnAutodesk, onOpenLink,
 ...
 const activeProjectId = currentElementObj?.projetoBimId && currentElementObj?.projetoBimId !== 'N/A' ? currentElementObj.projetoBimId : projetoBimId;
 ...
 <FontAwesomeIcon icon={faLayerGroup} className="text-sm"/> Prop.
 </button>
 ...
 <button onClick={() => setExpandedRowId(isExpanded ? null : displayId)} className="w-full text-left p-2.5 flex items-center justify-between hover:bg-blue-50 transition-colors"
 >
 <div className="flex items-center gap-2 overflow-hidden flex-1">
 <FontAwesomeIcon icon={isExpanded ? faChevronDown : faChevronRight} className="text-[10px] text-gray-400 shrink-0 w-3" />
 ...
 <BimElementBudget projetoBimId={activeProjectId} selectedIds={[displayId]} />
 </div>
 )}
 </div>
 );
 ...
 <BimProperties viewer={viewer}
 selectedIds={selectedIdsOnly} elementExternalId={safeId} selectedCount={selectedCount}
 projetoBimId={activeProjectId} urnAutodesk={urnAutodesk} onOpenLink={onOpenLink}
 ...
 <BimElementPlanning elementExternalId={safeId} // Fallback para single
 selectedIds={selectedIdsOnly} // ARRAY DE IDS
 selectedCount={selectedCount} // CONTAGEM
 projetoBimId={activeProjectId} elementName={selectedCount > 1 ? `${selectedCount} Itens` : `ID: ${String(safeId).substring(0, 8)}`} onOpenLink={onOpenLink} onOpenCreate={onOpenCreate} />
 )}
 ...
 <button onClick={() => onOpenNote({ externalId: safeId, projetoBimId: activeProjectId })} className="w-full bg-purple-600 hover:bg-purple-700 text-white py-2.5 rounded-lg text-xs font-bold shadow-md shadow-purple-100 active:scale-95 transition-all flex items-center justify-center gap-2"
 >
