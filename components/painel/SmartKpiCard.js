...
import { faSpinner, faExclamationCircle, faEdit, faTrash, faCalculator, faCopy, faExternalLinkAlt } from '@fortawesome/free-solid-svg-icons';
...
export default function SmartKpiCard({ kpi, onEdit, onDelete, onDuplicate }) {
 ...
 try {
 const cleanFormula = formulaFinal.replace(/[^0-9\.\+\-\*\/\(\)\s]/g, ''); // eslint-disable-next-line no-new-func
 ...
 {tipo === 'filtro' && (
 <button onClick={handleVerLancamentos} className="text-gray-400 hover:text-blue-600 p-1 bg-white rounded-md shadow-sm border border-gray-100 hover:border-blue-200"
 ...
 {onDuplicate && (
 <button onClick={(e) => { e.stopPropagation(); onDuplicate(kpi); }} className="text-gray-400 hover:text-green-500 p-1"
 title="Duplicar KPI"
 >
 ...
 {onEdit && (
 <button onClick={(e) => { e.stopPropagation(); onEdit(kpi); }} className="text-gray-400 hover:text-blue-500 p-1"
 title="Editar KPI"
 >
 ...
 {onDelete && (
 <button onClick={(e) => { e.stopPropagation(); onDelete(kpi); }} className="text-gray-400 hover:text-red-500 p-1"
 title="Excluir KPI"
 >
