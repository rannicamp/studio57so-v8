...
import { faTimes, faSave, faCheck, faChartPie, faArrowUp, faArrowDown, faWallet, faMoneyBillWave, faPiggyBank, faCoins, faSpinner, faPen, faCalculator, faDatabase, faPercentage, faLayerGroup, faChartLine, faChartBar, faSquare, faCalendarAlt, faAlignLeft
...
export default function KpiBuilderModal({ isOpen, onClose, onSaveSuccess, kpiToEdit = null }) {
 ...
 const [formulaVisual, setFormulaVisual] = useState(''); const [formatoSaida, setFormatoSaida] = useState('moeda');
 ...
 const kpisDisponiveis = useMemo(() => todosKpis.filter(k => k.id !== kpiToEdit?.id), [todosKpis, kpiToEdit]);
 ...
 const tag = ` [${kpiNome}] `; setFormulaVisual(prev => prev + tag);
 };
 ...
 await queryClient.invalidateQueries({ predicate: (query) => query.queryKey[0]?.toString().includes('kpi') });

 if (onSaveSuccess) onSaveSuccess(); onClose();
 } catch (error) {
 ...
 <input type="text" value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Ex: Faturamento Mensal..." className="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" autoFocus={!kpiToEdit} />
 </div>
 <div>
 <label className="block text-sm font-medium text-gray-700 mb-2">
 <FontAwesomeIcon icon={faLayerGroup} className="mr-1 text-gray-400"/> Grupo
 </label>
 <input list="grupos-sugestoes"
 type="text" value={grupo} onChange={(e) => setGrupo(e.target.value)} placeholder="Ex: Vendas" className="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none bg-gray-50 focus:bg-white transition-colors" />
 ...
 <div>
 <label className="block text-sm font-medium text-gray-700 mb-2">
 <FontAwesomeIcon icon={faAlignLeft} className="mr-1 text-gray-400"/> Descrição Curta <span className="text-xs text-gray-400 font-normal">({descricao.length}/60 car.)</span>
 </label>
 <input type="text" value={descricao} onChange={(e) => setDescricao(e.target.value)} maxLength={60}
 placeholder="Ex: Total de vendas recebidas este mês. Aparece no rodapé do card." className="w-full p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" />
 </div>
 ...
 <button onClick={() => setTipoVisualizacao('card')}
 ...
 <button onClick={() => setTipoVisualizacao('grafico_linha')}
