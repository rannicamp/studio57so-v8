...
import { LineChart, Line, BarChart, Bar, XAxis, CartesianGrid, Tooltip, ResponsiveContainer, Area, AreaChart, ReferenceLine, Cell
} from 'recharts';
...
const LARGURA_ITEM_PX = 60; export default function SmartChartCard({ kpi, onEdit, onDelete }) {
 ...
 queryKey: ['chart_data_v8', kpi?.id, filtros, user?.organizacao_id], queryFn: async () => {
 ...
 enabled: !!user?.organizacao_id && !!kpi, staleTime: 1000 * 60 * 5, });
 ...
 const dataParaUso = item.data_ref; if (!dataParaUso) return acc;
 ...
 acc[chave] = { chave, dataOrdenacao: dataVisual.getTime(), dataObj: dataVisual, valor: 0, label: '' };
 try {
 ...
 acc[chave].valor += despesaItem; } else {
