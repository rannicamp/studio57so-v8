...
const fetchKpis = async () => {
    ...
    if (!user) return [];
    const { data, error } = await supabase.from('kpis_personalizados').select('*').eq('usuario_id', user.id).order('grupo').order('created_at', { ascending: false });
...
export default function ConstrutorKpiManager() {
    ...
    const mutationOptions = {
        onSuccess: (message) => {
            ...
            queryClient.invalidateQueries({ queryKey: ['customKpiDefinitions'] });
        },
    ...
    const deleteMutation = useMutation({
        mutationFn: async (kpiId) => {
            const supabase = createClient();
            const { error } = await supabase.from('kpis_personalizados').delete().eq('id', kpiId);
            if (error) throw new Error(error.message);
    ...
    const toggleVisibilityMutation = useMutation({
        mutationFn: async ({ kpiId, newValue }) => {
            const supabase = createClient();
            const { error } = await supabase.from('kpis_personalizados').update({ exibir_no_painel: newValue }).eq('id', kpiId);
            if (error) throw new Error(error.message);
    ...
    const handleDelete = (kpiId) => { if (window.confirm('Tem certeza que deseja excluir este KPI permanentemente?')) deleteMutation.mutate(kpiId); };
