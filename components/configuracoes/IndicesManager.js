                ...
                <div className="flex flex-wrap gap-2 bg-gray-100 p-1.5 rounded-lg max-w-full">
                    {['IPCA', 'INPC', 'IGP-M', 'IGP-DI', 'INCC', 'IPC-FIPE', 'SELIC', 'CDI', 'TR'].map(indice => (
                        <button 
                            ...
                            className={`px-3 py-1.5 text-xs sm:text-sm font-bold rounded-md transition-all ${selectedIndice === indice ? 'bg-white shadow-sm text-blue-700' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-200'}`}
                        >
                            {indice}
                        </button>
                    ))}
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                            {indices.map((item) => (
                                <tr key={item.id} className="hover:bg-blue-50/30 transition-colors group">
                                    ...
                                    {exibirColunaAnual && (
                                        <td className="px-6 py-4 whitespace-nowrap text-right">
                                            <span className={`text-sm font-extrabold ${item.taxa_anual_equivalente < 0 ? 'text-red-600' : 'text-orange-600'}`}>
