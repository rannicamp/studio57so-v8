...
const ResetPasswordModal = ({ isOpen, onClose, user }) => {
 ...
 setIsLoading(false);
 if (result.success) { toast.success(result.message); onClose(); setPassword(''); } else { toast.error(result.message); }
 };
...
const ForceUnlockModal = ({ isOpen, onClose, user, organizationId }) => {
 ...
 <button type="button" onClick={handleUnlock} disabled={isLoading} className="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg transition-colors flex items-center justify-center gap-2"
 >
...
export default function UserManagementForm({ initialUsers, allEmployees, allRoles, organizationId }) {
 ...
 filterRole: 'all',
 filterStatus: 'all',
 sortKey: 'ultimo_acesso', sortDirection: 'desc'
 });
 ...
 const matchesStatus = uiState.filterStatus === 'all' ? true :
 ...
 break;
 case 'ultimo_acesso': valA = new Date(a.ultimo_acesso || 0).getTime();
 ...
 <th className={`px-6 py-3 text-${align} text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors select-none group`}
 ...
 <input type="text" placeholder="Buscar por nome ou email..." value={uiState.search}
 ...
 <select value={uiState.filterRole}
 ...
 <div className="relative w-full sm:w-auto">
 <select value={uiState.filterStatus}
 ...
 {/* Função */}
 <td className="px-6 py-4 whitespace-nowrap">
 <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full border ${['Administrador', 'Proprietário'].includes(user.funcao?.nome_funcao)
 ? 'bg-purple-100 text-purple-800 border-purple-200 dark:bg-purple-900/30 dark:text-purple-300' : user.funcao?.nome_funcao === 'Corretor'
 ...
 {/* Status Switch */}
 <td className="px-6 py-4 whitespace-nowrap">
 <button onClick={() => toggleStatusMutation.mutate({ userId: user.id, currentStatus: user.is_active })}
 disabled={toggleStatusMutation.isPending}
 className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${user.is_active ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600'}`}
 >
 ...
 <button onClick={() => setUnlockModalUser(user)}
 ...
 <button onClick={() => setPasswordModalUser(user)}
