 ...
 *,
 ...
 cargos ( id, nome ), documentos_funcionarios ( id, nome_documento, caminho_arquivo, tipo:tipo_documento_id(sigla) )
 `)
 ...
 let pathStr = employee.foto_url;

 // Limpa qualquer formato de URL do Supabase Storage para obter o caminho relativo
 ...
 `/storage/v1/object/${BUCKET_NAME}/`,
 ];
 for (const pattern of patterns) {
 if (pathStr.includes(pattern)) {
 pathStr = pathStr.split(pattern)[1].split('?')[0];
 break;
 }
 }

 if (pathStr.startsWith('http')) {
 // Já é uma URL externa (ex: avatar do Google) ou formato desconhecido — usa direto
 employee.foto_url = pathStr;
 } else {
 // É um caminho relativo correto, geramos a Signed URL
 ...
 console.warn(`Foto não carregada para ${employee.full_name}: path="${pathStr}"`, urlError?.message);
