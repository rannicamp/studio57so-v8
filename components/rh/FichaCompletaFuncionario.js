...
const DocumentosSection = ({ documentos: initialDocuments, employeeId, employeeName, organizacaoId, user, onUpdate }) => {
 ...
 <GerenciadorAnexosGlobal anexos={documentos.map(doc => ({
 id: doc.id,
 ...
 <FilePreviewModal isOpen={!!previewAnexo}
...
const FinanceiroSection = ({ lancamentos, onEditLancamento }) => {
 ...
 <td className="px-4 py-2 text-center">
 <button onClick={() => onEditLancamento(lanc)} title="Editar Lançamento" className="text-blue-500 hover:text-blue-700 transition-colors">
 ...
 */}
 ...
 <input type="month" value={selectedMonthForPonto} onChange={(e) => setSelectedMonthForPonto(e.target.value)} className="border border-gray-300 p-2 rounded-lg bg-gray-50 focus:ring-2 focus:ring-blue-500 focus:outline-none font-bold text-blue-700 cursor-pointer shadow-inner" />
 </div>
 </div>
