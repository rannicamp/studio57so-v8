...
import { faFileContract, faPlus, faCalendarAlt, faMoneyBillWave, faSpinner,
 faTrash,
 ...
 *,
 ...
 return nomeFornecedor.toLowerCase().includes(termo) || titulo.toLowerCase().includes(termo);
 });
 ...
 <ContratoCard key={contrato.id} contrato={contrato} onDelete={() => {
 ...
 {/* Modal */}
 <NovoContratoModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}
...
function ContratoCard({ contrato, onDelete }) {
 ...
 // Formatação de Moeda
 const valorFormatado = contrato.valor_total ? new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(contrato.valor_total)
 : 'A combinar';
 ...
 <span>
 <span className="font-medium">Vigência:</span> {formatDate(contrato.data_inicio)} {contrato.data_fim ? ` até ${formatDate(contrato.data_fim)}` : ' (Indeterminado)'}
 </span>
 </div>
 <div className="flex items-center gap-2">
 ...
 <button onClick={handleOpenDossier}
