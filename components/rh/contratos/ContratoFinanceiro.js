...
export default function ContratoFinanceiro({ contrato }) {
 ...
 .from('lancamentos')
 .select(`
 id, descricao, valor, data_vencimento, data_pagamento,
 data_transacao,
 status, tipo,
 ...
 <span className={`px-2 py-1 text-xs font-semibold rounded-full ${isPago ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
 }`}>
