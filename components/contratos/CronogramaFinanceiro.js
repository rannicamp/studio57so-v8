...
export default function CronogramaFinanceiro({ contrato, onUpdate }) {
  ...
  // =================================================================================
  // LÓGICA "INTELIGENTE" (Adaptada para incluir Saldo Remanescente na listagem ordenada)
  // =================================================================================
  const {
    parcelasNormais,
    parcelasOrdenadas,
    ...
    let dSaldo = {
      ...
      status_pagamento: pSaldoOriginal?.status_pagamento || 'Pendente',
      lancamento_id: pSaldoOriginal?.lancamento_id || null
    };
    ...
    // Adiciona o saldo remanescente à listagem ordenada cronologicamente
    const todasParcelas = [...pNormais];
    if (pSaldoOriginal || valorCalculadoSaldo > 0) {
      todasParcelas.push(dSaldo);
    }

    const pOrdenadas = todasParcelas.sort((a, b) => {
    ...
    return {
      ...
      diferencaDeSincronia: tDiferenca
    };
  ...
  const handleSaveEditingParcela = async (parcelaId) => {
    ...
    let targetParcelaId = parcelaId;

    if (parcelaId === 'calculado') {
      // Se for a parcela virtual de saldo remanescente, insere no banco
      const { data: insertedData, error: insertError } = await supabase.from('contrato_parcelas').insert({
        contrato_id: contratoId,
        descricao: updateData.descricao || 'Saldo Remanescente (Chaves)',
        tipo: 'Saldo Remanescente',
        data_vencimento: updateData.data_vencimento,
        valor_parcela: updateData.valor_parcela,
        organizacao_id: organizacaoId
      }).select().single();

      if (insertError) {
        toast.error("Erro ao criar saldo remanescente: " + insertError.message);
        setLoading(false);
        return;
      }
      targetParcelaId = insertedData.id;
    } else {
            ...
            <table className="min-w-full divide-y divide-gray-100 text-sm">
              ...
              <tbody className="bg-white divide-y divide-gray-50">
                {parcelasOrdenadas.map(p => {
                  ...
                  const isSaldoRem = p.tipo === 'Saldo Remanescente' || p.descricao?.toLowerCase().includes('saldo remanescente');
                  let dateClass = statusInfo.text === 'Atrasada' ? 'text-red-500 font-bold' : 'text-gray-500 font-medium';

                  // Estilos customizados para a linha de Saldo Remanescente
                  const trClass = isSaldoRem
                    ? "bg-blue-50/10 hover:bg-blue-50/20 font-semibold border-l-2 border-l-blue-500 transition-colors group"
                    ...
                    editingParcelaId === p.id ? (
                      <tr key={p.id} className="bg-blue-50/30">
                        ...
                        <td className="px-4 py-3">
                          <input
                            type="text"
                            value={editingParcelaData.descricao}
                            onChange={e => handleEditingParcelaChange('descricao', e.target.value)}
                            className="w-full p-2 bg-white border border-blue-400 text-sm font-bold text-gray-700 rounded-md focus:outline-none transition-colors"
                          />
                        </td>
                        <td className="px-4 py-3 font-semibold text-gray-600">{p.tipo}</td>
                        <td className="px-4 py-3">
                          <input
                            type="date"
                            value={formatDateForInput(editingParcelaData.data_vencimento)}
                            onChange={e => handleEditingParcelaChange('data_vencimento', e.target.value)}
                            className="w-full p-2 bg-white border border-blue-400 text-sm font-bold text-gray-700 rounded-md focus:outline-none transition-colors"
                          />
                        </td>
                        <td className="px-4 py-3">
                          <IMaskInput
                            ...
                            onAccept={(value) => handleEditingParcelaChange('valor_parcela', value)}
                            disabled={isSaldoRem}
                            className={`w-full p-2 border text-sm font-bold rounded-md focus:outline-none transition-colors text-right ${
                              isSaldoRem
                        ...
                        <td className="px-4 py-3 text-center font-bold text-gray-700">{p.status_pagamento}</td>
                        <td className="px-4 py-3 text-center">
                          <div className="flex justify-center items-center gap-2">
                            <button onClick={() => handleSaveEditingParcela(p.id)} disabled={loading} className="w-7 h-7 inline-flex items-center justify-center rounded-md bg-green-600 text-white hover:bg-green-700 transition-colors" title="Salvar"><FontAwesomeIcon icon={faSave} size="xs" /></button>
                            <button onClick={handleCancelEditingParcela} className="w-7 h-7 inline-flex items-center justify-center rounded-md bg-white border border-gray-200 text-gray-400 hover:text-gray-600 transition-colors" title="Cancelar"><FontAwesomeIcon icon={faTimes} size="xs" /></button>
                          </div>
                        </td>
                      </tr>
                    ) : (
                      <tr key={p.id} className={trClass}>
                        <td className="px-4 py-3 text-center">
                          {p.lancamento_id && <FontAwesomeIcon icon={faLink} size="xs" className="text-green-500" title={`Vinculado #${p.lancamento_id}`} />}
                        </td>
                        <td className="px-4 py-3 font-bold text-gray-700 flex items-center">
                          {p.descricao}
                          {isSaldoRem && (
                        ...
                        <td className="px-4 py-3 text-gray-500 font-medium">{p.tipo}</td>
                        <td className={`px-4 py-3 whitespace-nowrap ${dateClass}`}>
                          {formatDateForDisplay(p.data_vencimento)}
                        </td>
                        <td className={`px-4 py-3 text-right font-bold ${isSaldoRem ? 'text-blue-600' : 'text-gray-800'}`}>
                          {formatCurrency(p.valor_parcela)}
                        </td>
                        <td className="px-4 py-3 text-center">
                          <span className={`px-2 py-0.5 text-[10px] font-bold rounded-full border shadow-sm ${statusInfo.className.replace('text-800', 'text-600').replace('bg-100', 'bg-50')}`}>
                            {statusInfo.text}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-center">
                          <div className="flex justify-center items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            {!isSaldoRem && (
                              <button onClick={() => handleDuplicateParcela(p.id)} disabled={loading} className="w-7 h-7 inline-flex items-center justify-center rounded-md bg-white border border-gray-200 text-gray-400 hover:text-blue-600 hover:border-blue-300 transition-all shadow-sm" title="Duplicar"><FontAwesomeIcon icon={faCopy} size="xs" /></button>
                            )}
                            {p.status_pagamento === 'Pendente' && (
                              <button onClick={() => handleStartEditingParcela(p)} className="w-7 h-7 inline-flex items-center justify-center rounded-md bg-white border border-gray-200 text-gray-400 hover:text-blue-600 hover:border-blue-300 transition-all shadow-sm" title="Editar"><FontAwesomeIcon icon={faEdit} size="xs" /></button>
                            )}
                            {!isSaldoRem && (
                              <button onClick={() => handleDeleteParcela(p.id)} disabled={loading} className="w-7 h-7 inline-flex items-center justify-center rounded-md bg-white border border-gray-200 text-gray-400 hover:text-red-600 hover:border-red-300 transition-all shadow-sm" title="Excluir"><FontAwesomeIcon icon={faTrash} size="xs" /></button>
                            )}
                          </div>
                        </td>
                      </tr>
                ...
                <tr className="bg-gray-50/50 border-t border-gray-100">
                  <td className="px-4 py-3"></td>
                  <td className="px-4 py-3"><input type="text" placeholder="Adicionar parcela adicional..." value={newParcela.descricao} onChange={e => setNewParcela({ ...newParcela, descricao: e.target.value })} className="w-full p-2 bg-white border border-gray-300 text-sm font-medium text-gray-700 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 transition-colors" /></td>
                  <td className="px-4 py-3 text-gray-500 font-medium">Adicional</td>
                  ...
                  <td className="px-4 py-3 text-center text-gray-400 font-bold text-[10px] uppercase">A Pagar</td>
                  <td className="px-4 py-3 text-center"><button onClick={handleAddParcela} disabled={loading} className="bg-blue-600 text-white px-4 py-2 rounded-md font-bold hover:bg-blue-700 disabled:bg-gray-300 transition-colors text-xs">{loading ? <FontAwesomeIcon icon={faSpinner} spin /> : <><FontAwesomeIcon icon={faPlus} /> Add</>}</button></td>
                </tr>
              </tbody>
