...
export default function FichaContrato({
 ...
 disabled={isDisabled}
 title={isDisabled ? "Defina um cliente na aba 'Venda' para habilitar" : ""}
 className={`flex items-center gap-2 py-2 px-4 font-semibold text-sm transition-all duration-200 outline-none whitespace-nowrap rounded-t-lg border border-transparent
 ${activeTab === tabId ? 'bg-white text-blue-600 border-gray-200 border-b-white -mb-px z-10' : 'bg-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-100'}
 ${isDisabled ? 'opacity-40 cursor-not-allowed' : ''}`
 }
 >
 <FontAwesomeIcon icon={isDisabled ? faLock : icon} className={activeTab === tabId ? 'text-blue-500' : 'text-gray-400'} /> {label}
 </button>
 );
 };
 ...
 <div className="print:hidden bg-white p-6 rounded-lg shadow-md border border-gray-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
 <div className="flex items-start gap-4">
 <div className="w-12 h-12 rounded-lg bg-gray-50 border border-gray-200 flex items-center justify-center text-gray-500 flex-shrink-0">
 ...
 contrato.status_contrato === 'Em assinatura' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' :
 contrato.status_contrato === 'Em negociação' ? 'bg-blue-100 text-blue-700 border-blue-200' :
 'bg-gray-100 text-gray-600 border-gray-200'
 }`}>
 {contrato.status_contrato}
 </span>
 </div>
 <div className="text-sm font-medium text-gray-500 space-y-1 mt-1">
 <p className="flex items-center gap-2">Cliente: <span className="text-gray-800 font-semibold">{contrato.contato?.nome || contrato.contato?.razao_social || 'NÃO DEFINIDO'}</span></p>
 <p className="flex items-center gap-2">Empreendimento: <span className="text-gray-800 font-semibold">{contrato.empreendimento?.nome}</span></p>
 </div>
 </div>
 </div>

 <div className="w-full md:w-auto">
 <label htmlFor="modeloContratoSelect" className="block text-xs font-semibold text-gray-500 mb-1 uppercase">Modelo de Cláusulas</label>
 <div className="flex items-center gap-2">
 <select
 id="modeloContratoSelect"
 value={contrato.modelo_contrato_id || ''}
 onChange={handleModeloChange}
 className="w-full md:w-64 p-2 bg-white border border-gray-300 rounded-md text-sm font-medium focus:ring-1 focus:ring-blue-500 outline-none transition-all"
 disabled={loadingModelos || updateModeloMutation.isPending || modelosContrato.length === 0}
 >
 <option value="">{loadingModelos ? 'Carregando...' : (modelosContrato.length === 0 ? 'Nenhum disponível' : '-- Selecione --')}</option>
 ...
 <KpiCard title="Valor do Contrato" value={kpiData.valorTotal} icon={faFileSignature} />
 <KpiCard title="Total Pago" value={kpiData.totalPago} icon={faCheckCircle} />
 <KpiCard title="Saldo Devedor" value={kpiData.saldoDevedor} icon={faDollarSign} />
 <KpiCard title="Próxima Parcela" value={kpiData.proximaParcela} icon={faCalendarCheck} />
 </div>
 ...
 <TabButton tabId="resumo" label="Venda" icon={faHandshake} />
 {contrato.tipo_documento === 'CONTRATO' && (
 <TabButton tabId="cronograma" label="Cronograma" icon={faFileInvoiceDollar} />
 )}
 <TabButton tabId="financeiro" label="Lançamentos" icon={faMoneyCheckDollar} />
 <TabButton tabId="gerador" label="Gerar PDF" icon={faFileContract} />
