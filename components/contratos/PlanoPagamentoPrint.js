...
export default function PlanoPagamentoPrint({ contrato, signatory, geradoPor }) {
 ...
 const parcelasNormais = parcelas.filter(p => p.tipo !== 'Saldo Remanescente' && !p.descricao?.toLowerCase().includes('saldo remanescente')
 );
 const parcelaSaldoOriginal = parcelas.find(p => p.tipo === 'Saldo Remanescente' || p.descricao?.toLowerCase().includes('saldo remanescente')
 );
 ...
 const { contato: cliente, produtos = [], empreendimento, valor_final_venda: valorTotalContrato } = contrato;
