...
export default function MergeModal({ isOpen, onClose, contactsToMerge, onMergeComplete }) {
 ...
 // Lógica simples: quem tem ID menor é mais antigo. // Poderíamos refinar para quem tem mais campos preenchidos.
 return a.id - b.id;
 });
 ...
 <select value={primaryContactId || ''} onChange={(e) => setPrimaryContactId(parseInt(e.target.value))} className="w-full p-2 border border-green-300 rounded-md bg-green-50 text-green-900 font-medium focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none"
 >
