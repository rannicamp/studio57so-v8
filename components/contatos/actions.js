...
export async function saveContactAction({ formData, isEditing }) {
 ...
 .from('usuarios') .select('organizacao_id')
 .eq('id', user.id)
 .maybeSingle();
 ...
 let cleanNumber = tel.telefone.replace(/\D/g, ''); const ddi = (tel.country_code || '+55').replace('+', '');
 ...
 tipo: tel.tipo || 'Celular', organizacao_id
 };
 });
 ...
 tipo: mail.tipo || 'Pessoal', organizacao_id
 }));
