...
export default function ContatoList({ initialContatos, onActionComplete, onRowClick,
 ...
 case 'telefone': return formatPhoneNumber(contato.telefone); case 'email': return contato.email || 'N/A';
 case 'display_name': // LÓGICA HÍBRIDA: PC vs Celular
 return (
 ...
 <button onClick={() => setIsMergeModalOpen(true)} className="p-2 border rounded-md w-full md:w-auto shadow-sm bg-blue-600 text-white hover:bg-blue-600 flex items-center justify-center gap-2">
 ...
 <tr>
 <th className="px-4 py-3 text-left sticky top-0 bg-gray-50 z-10 shadow-sm w-10"> <input type="checkbox" className="h-4 w-4 rounded" onChange={handleSelectAll} checked={sortedAndFilteredContatos.length > 0 && selectedContatos.length === sortedAndFilteredContatos.length} title="Selecionar todos visíveis"/> </th>
 {allColumns.map(col => (
 ...
 {allColumns.map(col => (
 visibleColumns[col.key] && ( <td key={col.key} className={`px-6 py-4 text-sm ${col.className}`}> {getColumnValue(contato, col.key)} </td> )
 ))}
