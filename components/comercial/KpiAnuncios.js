...
export default function KpiAnuncios({ data, isLoading }) {
  ...
  const kpis = useMemo(() => {
    if (!data || data.length === 0) {
      return { totalGasto: 0, totalLeads: 0, cpl: 0, cpm: 0, anunciosAtivos: 0 };
    }
    ...
    return {
      ...
      cpm: totalImpressions > 0 ? (totalGasto / totalImpressions) * 1000 : 0,
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
        {[1, 2, 3, 4, 5].map(i => <div key={i} className="bg-gray-100 h-24 rounded-xl animate-pulse border border-gray-200"></div>)}
      </div>
    );
  }
  ...
  const cards = [
    ...
    { title: "Custo por Mil Impressões (CPM)", value: formatBRL(kpis.cpm), icon: faCoins, color: "text-fuchsia-600", bg: "bg-fuchsia-50" },
    { title: "Anúncios Ativos", value: kpis.anunciosAtivos, icon: faBullhorn, color: "text-indigo-600", bg: "bg-indigo-50" },
  ];
