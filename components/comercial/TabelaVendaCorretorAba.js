...
export default function TabelaVendaCorretorAba({ empreendimentoId, organizacaoId }) {
 ...
 const produtosFiltrados = filtroStatus ? data.produtos.filter(p => (p.status || 'Disponível').trim().toLowerCase() === filtroStatus.trim().toLowerCase()) : data.produtos;
 ...
 <TabelaVenda produtos={produtosFiltrados} config={data.configuracoes} parcelasAdicionais={data.parcelasAdicionais} empreendimento={data.empreendimento} />
 </div>
 )}
 </div>
 );
