...
export default function FiltroAnuncios({ filters, setFilters, campaigns, adsets,
 ...
 if (period === 'today') { startDate = endDate = today; } else if (period === 'week') {
 ...
 setFilters(prev => ({ ...initialFilterState, ...filterSettings, searchTerm: prev.searchTerm }));
 setIsFilterMenuOpen(false);
 };
 ...
 <MultiSelectDropdown label="Status" options={statusOptions} selectedIds={filters.status} onChange={(selected) => handleFilterChange('status', selected)} placeholder="Todos os Status"
 />
 <MultiSelectDropdown label="Campanhas" // Usando as nossas listas formatadas com 'nome'
 options={campanhasFormatadas} selectedIds={filters.campaignIds} onChange={(selected) => handleFilterChange('campaignIds', selected)} placeholder="Todas as Campanhas"
 />
 <MultiSelectDropdown label="Conjuntos de Anúncios" // Usando as nossas listas formatadas com 'nome'
 options={conjuntosFormatados} selectedIds={filters.adsetIds} onChange={(selected) => handleFilterChange('adsetIds', selected)} placeholder="Todos os Conjuntos"
 />
 </div>
 ...
 const isActive = filters.status === f.settings.status && filters.campaignIds === f.settings.campaignIds; return (
