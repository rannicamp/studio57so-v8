...
export default function RdoForm({ selectedEmpreendimento }) {
  ...
  useEffect(() => {
    const loadRdoData = async () => {
      ...
      try {
        ...
        // AGORA BUSCA OCORRÊNCIAS E FOTOS JUNTAMENTE COM O RDO
        const { data: existingRdo, error: fetchRdoError } = await supabase
          .from('diarios_obra')
          .select('*, ocorrencias(*), rdo_fotos_uploads(*)') // Busca dados relacionados
        ...
        // Busca atividades e funcionários
        const { data: activitiesData, error: activitiesError } = await supabase.from('activities').select('id, nome, status').eq('empreendimento_id', selectedEmpreendimento.id).order('nome');
        if (activitiesError) throw activitiesError;
        setActivities(activitiesData || []);

        const { data: employeesData, error: employeesError } = await supabase.from('funcionarios').select('id, full_name').eq('empreendimento_atual_id', selectedEmpreendimento.id).order('full_name');
        ...
        if (existingRdo && existingRdo.length > 0) {
          ...
          const mergedActivities = (activitiesData || []).map(dbAct => {
              ...
              return {
                  id: dbAct.id,
                  nome: dbAct.nome,
                  status: rdoActivity ? rdoActivity.status : dbAct.status,
          ...
          const initialRdoData = {
            ...
            mao_de_obra: (employeesData || []).map(emp => ({ id: emp.id, name: emp.full_name, present: true, observacao: '' })),
            status_atividades: (activitiesData || []).map(act => ({ id: act.id, nome: act.nome, status: act.status, observacao: '' })),
          };
  ...
  useEffect(() => {
    if (rdoFormData.id && !isRdoLocked) {
      debouncedSaveRdoMain({
        ...
        status_atividades: activityStatuses,
      });
    }
  ...
  const handleActivityStatusChange = async (activityId, newStatus, newObservation) => {
    ...
    const updatedActivityStatuses = activityStatuses.map(act => act.id === activityId ? { ...act, status: newStatus, observacao: newObservation } : act);
    setActivityStatuses(updatedActivityStatuses);
    setMessage('Atualizando atividade...');
    const { error } = await supabase.from('activities').update({ status: newStatus }).eq('id', activityId);
  ...
  // --- NOVAS FUNÇÕES PARA OCORRÊNCIAS ---
  ...
  const handleAddPhoto = async () => {
    ...
    const filePath = `${currentUser.id}/${rdoFormData.id}/${Date.now()}-${currentPhotoFile.name}`;
    ...
    <div className="bg-white p-6 rounded-lg shadow">
      ...
      <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
        ...
        <div className="border-b border-gray-200 pb-4">
            ...
            {employeePresences.length === 0 ? <p className="text-gray-500">Nenhum funcionário alocado.</p> : (
            <ul className="divide-y divide-gray-200">
                ...
                <li key={employee.id} className="py-3">
                    ...
                    <div className="flex items-center gap-2 w-full md:w-1/5">
                        <button type="button" onClick={() => { if (!isRdoLocked) setEmployeePresences(prev => prev.map(emp => emp.id === employee.id ? { ...emp, present: !emp.present } : emp)); }} disabled={isRdoLocked} className={`relative inline-flex h-6 w-11 items-center rounded-full ${employee.present ? 'bg-green-500' : 'bg-red-500'}`}>
        ...
        {/* Seção 5: Ocorrências do Dia - REATIVADA */}
        <div className="border-b border-gray-200 pb-4">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-3">
            ...
            <button type="button" onClick={handleAddOccurrence} disabled={isRdoLocked || !rdoFormData.id || !currentNewOccurrence.descricao} className="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600 disabled:bg-gray-400">Adicionar</button>
          </div>
        ...
        {/* Seção 6: Fotos do Dia - REATIVADA */}
        <div>
            ...
            <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2">
              {allPhotosMetadata.map((photo) => (
                <div key={photo.id} className="relative group border rounded-md overflow-hidden">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white text-xs p-1 truncate">{photo.descricao}</div>
