...
const AVATAR_COLORS = [
 ...
 'bg-lime-600', 'bg-amber-500', 'bg-blue-600', 'bg-red-500',
];
...
const BroadcastListItem = ({ list, onSelect, onDelete, isSelected }) => (
 ...
 className={`relative group p-4 border-b cursor-pointer hover:bg-gray-50 transition-colors bg-white ${isSelected ? 'bg-[#f0f2f5] border-l-4 border-l-blue-500' : ''
 }`}
 >
...
export default function ConversationList({ conversations, broadcastLists, isLoading, onSelectContact, selectedContactId, onSelectList, selectedListId }) {
 ...
 <div className="w-16 h-16 bg-[#00a884]/10 rounded-full flex items-center justify-center mb-4 text-[#00a884]">
 <FontAwesomeIcon icon={faInbox} className="text-3xl opacity-80" />
 </div>
 <h3 className="text-lg font-bold text-gray-800 mb-1">Nenhuma Conversa</h3>
 <p className="text-sm text-gray-500 mb-6">Parece que ainda não há histórico neste filtro.</p>
 <button onClick={() => setIsNewChatOpen(true)} className="bg-[#00a884] hover:bg-[#008f6f] text-white font-medium py-2 px-6 rounded-lg transition-colors flex items-center gap-2 shadow-sm text-sm">
 <FontAwesomeIcon icon={faPlus} /> Iniciar primeira conversa
 </button>
 </div>
 </div>
 ) : (
 ...
 <FontAwesomeIcon icon={faBullhorn} className="text-3xl opacity-80" />
 </div>
 <h3 className="text-lg font-bold text-gray-800 mb-1">Sem Listas</h3>
 <p className="text-sm text-gray-500 mb-6 px-4">Crie listas segmentadas para disparos em massa e comunicações direcionadas.</p>
 <button onClick={() => setIsNewListOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-6 rounded-lg transition-colors flex items-center gap-2 shadow-sm text-sm">
 ...
 failedConversations.length === 0 ? (
 <div className="flex flex-col items-center justify-center p-8 text-center h-[50vh] animate-in fade-in zoom-in duration-300">
 <div className="bg-green-50/50 border border-green-100 rounded-2xl p-8 max-w-sm w-full mx-auto shadow-sm flex flex-col items-center">
 <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mb-4 text-green-500">
 <FontAwesomeIcon icon={faExclamationCircle} className="text-3xl opacity-80" />
 </div>
 <h3 className="text-lg font-bold text-gray-800 mb-1">Tudo OK!</h3>
 <p className="text-sm text-gray-500">Nenhum erro de envio registrado recentemente.</p>
 </div>
 </div>
 ) : (
 <ul className="flex-grow">
