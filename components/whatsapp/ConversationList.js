...
import { usePersistentState } from '@/hooks/usePersistentState'; // Garante a memória da aba
...
export default function ConversationList({ conversations, broadcastLists, isLoading, onSelectContact, selectedContactId, onSelectList, selectedListId }) {
  // Estado Persistente para a Aba (Chats ou Listas)
  ...
  // --- AÇÃO DE CONVERSA (Arquivar/Excluir) ---
  const handleAction = async (action, conversation, e) => {
    e.stopPropagation(); // Impede abrir a conversa ao clicar no menu
    ...
    try {
        // Tenta pegar o ID correto (compatibilidade com versões diferentes da API)
  ...
  const handleCreateAction = () => {
      if (activeTab === 'chats') {
          setIsNewChatOpen(true); // Modal Conversa Individual
      } else {
          setIsNewListOpen(true); // Modal Lista de Transmissão
      }
  };
  ...
  // Filtra conversas ativas e arquivadas com segurança
  const activeConversations = conversations?.filter(c => !c.is_archived) || [];
  const archivedConversations = conversations?.filter(c => c.is_archived) || [];

  // --- COMPONENTE: ITEM DE CONVERSA (Com Menu) ---
  const ConversationItem = ({ conversation, isArchivedList = false }) => {
    // Garante ID único para a key e seleção
    const uniqueId = conversation.conversation_id || conversation.id;
    // Verifica se é o selecionado (compara com contato_id ou conversation_id)
  ...
  // --- COMPONENTE: ITEM DE LISTA DE TRANSMISSÃO ---
                    ...
                    <>
                        <ul className="flex-grow">
                            {activeConversations.map((c) => (
                                <ConversationItem key={c.conversation_id || c.id} conversation={c} />
                            ))}
                        </ul>
                ...
                (!broadcastLists || broadcastLists.length === 0) ? (
                    ...
                    <ul className="flex-grow">
                        {broadcastLists.map((list) => (
                            <BroadcastListItem key={list.id} list={list} />
                        ))}
                    </ul>
                )
            )}
