...
export function useAudioRecorder(onSendAudio) {
 ...
 } catch (err) { console.error("Erro ao iniciar gravação:", err);
 ...
 if (audioContextRef.current) { audioContextRef.current.close().catch(e => console.log(e)); audioContextRef.current = null; }
 };
