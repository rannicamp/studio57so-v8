...
export default function LocationPickerModal({ isOpen, onClose, onSend }) {
 ...
 setSearchQuery(res.display_name.split(',')[0]); setSearchResults([]); };
 ...
 <button onClick={() => { onSend(position); onClose(); }}
 disabled={!position}
 ...
 >
 <FontAwesomeIcon icon={faPaperPlane} /> {position ? "Confirmar Localização" : "Selecione um local no mapa"}
 </button>
 </div>
 </div>
 </div>
