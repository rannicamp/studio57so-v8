...
export default function ContactProfile({ contact }) {
 ...
 <FontAwesomeIcon icon={faBullhorn} className="text-blue-600" /> Origem do Lead
 </h4>
 ...
 <textarea value={newNoteContent} onChange={(e) => setNewNoteContent(e.target.value)} placeholder="Escreva uma nota..." className="w-full p-2 border rounded-md text-sm bg-yellow-50 focus:bg-white transition-colors focus:ring-1 focus:ring-yellow-400 focus:border-yellow-400 outline-none resize-none" rows={2}
 ></textarea>
 <button onClick={() => addNoteMutation.mutate(newNoteContent)} disabled={addNoteMutation.isPending || !newNoteContent.trim()} className="absolute bottom-2 right-2 text-gray-400 hover:text-[#00a884] disabled:opacity-50 transition-colors"
 >
