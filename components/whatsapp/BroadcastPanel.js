...
import { faBullhorn, faUsers, faPaperPlane, faSpinner, faArrowLeft, faClock, faCheckCircle, faTimesCircle, faCalendarAlt, faEdit, faTrash, faPlay, faSync, faBolt, faLock, faEye, faCheckDouble,
 faPause, faStop, faTachometerAlt, faFilter } from '@fortawesome/free-solid-svg-icons';
...
const ProgressBar = ({ stats, total }) => {
 ...
 const readPct = Math.round((readCount / total) * 100); const deliveredPct = Math.round((deliveredCount / total) * 100);
...
export default function BroadcastPanel({ list, onBack }) {
 ...
 const [broadcasts, setBroadcasts] = useState([]); const [loading, setLoading] = useState(false);
 ...
 .select(`
 contatos (
 id, nome, telefones(telefone), tipo_contato,
 ...
 return {
 id: c.id, nome: c.nome, telefone: c.telefones?.[0]?.telefone, tipo: c.tipo_contato,
 ...
 return () => { supabase.removeChannel(channelMsg);
 ...
 if (b.status === 'paused') return { icon: faPause, color: 'text-blue-600', label: 'Pausado', bg: 'bg-blue-600 border-blue-600' };
 ...
 return { icon: faClock, color: isFuture ? 'text-yellow-600' : 'text-blue-600', label: isFuture ? 'Agendado' : 'Na Fila', bg: 'bg-yellow-50 border-yellow-100' };
 };
 ...
 {sending ? <div className="text-[#00a884]"><FontAwesomeIcon icon={faSpinner} spin size="2x" /><p>Processando...</p></div> : <button onClick={handleOpenNew} disabled={members.length === 0} className="bg-[#00a884] text-white px-6 py-3 rounded-lg hover:bg-[#008f6f] flex items-center justify-center gap-2 mx-auto disabled:opacity-50"><FontAwesomeIcon icon={faPaperPlane} /> Criar Mensagem</button>}
 </div>
 ...
 {hasPendingQueue && <button onClick={handleProcessQueue} disabled={isProcessingQueue} className="text-xs bg-blue-600 text-blue-600 px-3 py-1 rounded-full hover:bg-blue-600 font-bold flex items-center gap-2 animate-pulse">{isProcessingQueue ? <FontAwesomeIcon icon={faSpinner} spin /> : <FontAwesomeIcon icon={faPlay} />} Forçar Robô</button>}
 </div>
