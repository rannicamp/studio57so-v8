...
export default function EmpreendimentoCard({ empreendimento, organizacaoId }) {
 ...
 e.stopPropagation(); e.preventDefault(); const novoStatus = !isListed;
 // Atualiza visualmente primeiro (Optimistic UI)
 setIsListed(novoStatus); updateVendaStatus.mutate(novoStatus);
 };
 // Funções de estilo
 ...
 return (
 <Link href={`/empreendimentos/${empreendimento.id}`} className="block group rounded-lg overflow-hidden shadow-lg bg-white hover:shadow-xl transition-shadow duration-300 ease-in-out flex flex-col justify-between"
 >
 ...
 <div className="mt-3">
 <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${getStatusClass(statusLabel)}`}
 >
 {statusLabel}
 </span>
 </div>
 ...
 {/* ======================= BOTÃO TOGGLE ======================= */}
 <div className="p-4 border-t border-gray-200 mt-auto bg-gray-50 hover:bg-gray-100 transition-colors"
 ...
 <FontAwesomeIcon icon={isListed ? faStore : faStoreSlash} className={`mr-2 ${isListed ? 'text-green-500' : 'text-red-500'}`} />
