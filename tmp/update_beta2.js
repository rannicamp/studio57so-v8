...
// 1. Substituir os arrays (linhas 51 a 57, 0-indexed: 50 a 56)
...
lines.splice(50, 7, newArrays); // Remove 7 lines from index 50, insert newArrays

// 2. Encontrar os índices da seção "Layouts Inteligentes" dinamicamente
// Sabemos que começa com <section className="py-16 md:py-24 bg-black"> (cerca de linha 354, mas agora os índices mudaram devido à inserção)
// Vamos buscar a string exata
const startIndex = lines.findIndex(l => l.includes('<h2 className="text-3xl md:text-4xl font-bold mb-4 text-white">Layouts Inteligentes</h2>')) - 3;
const endIndex = lines.findIndex((l, idx) => idx > startIndex && l.includes('{/* --- GALERIA COMPLETA --- */}')) - 1;
...
lines.splice(startIndex, (endIndex - startIndex) + 1, novaSecao);

fs.writeFileSync(filePath, lines.join('\\n'), 'utf8');
console.log('Update successful! Real lines removed: ', (endIndex - startIndex) + 1);
