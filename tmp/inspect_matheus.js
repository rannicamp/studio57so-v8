...
async function main() {
  ...
  console.log('=== BUSCANDO CONTATO MATHEUS HENRIQUE ===');
  
  const contatosRes = await client.query(`
    SELECT id, nome, organizacao_id, created_at
    FROM contatos 
    WHERE nome ILIKE '%Matheus Henrique%'
  `);
  ...
  if (contatosRes.rows.length > 0) {
    ...
    const msgRes = await client.query(`
      SELECT id, contato_id, direction, content, status, sent_at, error_message, receiver_id, message_id, raw_payload
      FROM whatsapp_messages
      WHERE contato_id = ANY($1)
      ORDER BY sent_at ASC;
    `, [contatoIds]);

    console.log('\nMensagens do Matheus Henrique:');
