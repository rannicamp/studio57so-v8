...
BEGIN
    ...
    -- INTEGRAÇÃO INÉDITA FIX CORRIGIDO: Tratamento para CHAT INTERNO 
    IF (TG_TABLE_NAME = 'sys_chat_messages') THEN
        ...
        FROM public.sys_chat_participants -- Tabela correta
...
async function run() {
  const { error } = await supabase.rpc('execute_sql_query', { query: sql }).catch(() => ({error: null}));
  console.log('Fixed Trigger Name!');
}
run();
