...
async function run() {
  try {
    ...
    console.log("=== ELEMENTOS DO PROJETO 41 AGRUPADOS POR ORGANIZACAO_ID E IS_ACTIVE ===");
    const res = await client.query(`
      ...
      WHERE projeto_bim_id = 41
      GROUP BY organizacao_id, is_active
      ORDER BY total DESC
    `);
    console.table(res.rows);

    console.log("\n=== COMPARAÇÃO ESPECÍFICA DAS PAREDES PINTURA INTERNA ===");
    const resPintura = await client.query(`
      SELECT 
        organizacao_id, 
        is_active, 
        COUNT(*) as total
