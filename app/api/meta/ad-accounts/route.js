...
export async function GET(request) {
  try {
    ...
    console.error('⚠️ Erro ao buscar contas na API do Facebook (Rate Limit ou Conexão):', error.message);
    
    // BLINDAGEM DE RATE LIMIT: Se falhar a comunicação com o Facebook Graph,
    // mas nós já tivermos a conta pré-selecionada no banco de dados,
    // retornamos um fallback 200 OK com essa conta para não quebrar a tela inteira.
    try {
      const supabase = await createClient();
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        ...
        if (userData?.organizacao_id) {
          ...
          if (integracao?.ad_account_id) {
            const fallbackId = integracao.ad_account_id.startsWith('act_') 
              ? integracao.ad_account_id 
              : `act_${integracao.ad_account_id}`;
              
            console.log('🛡️ [FALLBACK ACTIVATED] Retornando conta salva no banco devido a falha na API da Meta:', fallbackId);
            
            return NextResponse.json({
              accounts: [{
                id: fallbackId,
                name: 'Incorporações (Modo de Segurança / Offline)',
                currency: 'BRL',
                status: 'Ativa',
                is_selected: true
              }],
              selected_account_id: fallbackId,
              is_fallback: true,
      ...
      console.error('❌ Falha ao processar fallback no banco de dados:', fallbackError.message);
    }

    return NextResponse.json({ error: error.message, accounts: [], selected_account_id: null }, { status: 500 });
  }
}
