...
export async function GET(request) {
    try {
        ...
        for (const indiceName of listaParaSincronizar) {
            try {
                // Acha o último mês fechado no BCB
                const novoDado = await buscarUltimoIndice(indiceName);
                ...
                if (novoDado) {
                    ...
                    resultados.erros.push(`API do BC não retornou nada para ${indiceName}.`);
                }
        ...
        return NextResponse.json({ 
            error: 'Falha interna na máquina cron.', 
            detalhe: fatalError.message ? fatalError.message : String(fatalError)
        }, { status: 500 });
    }
}
