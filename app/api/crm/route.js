...
async function sendTemplateMessage(config, to, contato, templateName, language) {
  ...
  try {
    ...
    // Auto-heal: Se falhar por erro de parâmetros (ex: template que não aceita variáveis no Meta)
    if (!response.ok && responseData.error?.code === 132000) {
      console.warn(`⚠️ [WhatsApp CRM] Detectado erro de parâmetros (132000) para o template '${templateName}'. Tentando reenvio automático sem parâmetros.`);
      
      // Limpa os componentes do payload
