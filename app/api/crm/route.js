...
    } catch (error) {
        console.error("API GET /api/crm: Erro CATCH geral:", error);
        return new NextResponse(JSON.stringify({ error: error.message }), { status: 500 });
    }
}
...
        const { funilId, nomeColuna, contatoIdParaFunil, action, contato_no_funil_id, contato_id, conteudo, usuario_id, empreendimentoId } = payload;
...
            const novoFunil = await createDefaultFunnel(supabase, empreendimentoId);
            if (!novoFunil) {
...
    } catch (error) {
        console.error("API POST /api/crm: Erro CATCH geral:", error);
        return new NextResponse(JSON.stringify({ error: error.message }), { status: 500 });
    }
}
...
                throw new Error("Não foi possível mover o contato devido a um erro no banco de dados.");
            }

            // Garante que 'data' é um array e verifica se alguma linha foi realmente atualizada
            const responseData = Array.isArray(data) ? data : []; 

            if (responseData.length === 0) {
                console.warn(`API PUT /api/crm (moveContact): Contato com ID ${payload.contatoId} não encontrado em 'contatos_no_funil' ou não houve alteração.`);
                // Retorna uma resposta de sucesso, mas indica que nenhuma alteração foi feita se o ID não for encontrado
                return NextResponse.json({ success: true, message: "Contato não encontrado ou já está na coluna de destino." });
            }

            console.log("API PUT /api/crm (moveContact): Contato movido com sucesso:", responseData);
            return NextResponse.json({ success: true, data: responseData, message: "Contato movido com sucesso!" });
