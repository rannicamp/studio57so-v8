...
// --- NOVO: ROTA PARA EDITAR (HORÁRIO, VARIÁVEIS, TEMPLATE) ---
export async function PUT(request) {
  ...
  try {
    ...
    console.error("Erro ao atualizar agendamento de transmissão:", error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}

// --- NOVO: ROTA PARA EXCLUIR AGENDAMENTO ---
export async function DELETE(request) {
  ...
  try {
    ...
    console.error("Erro ao excluir agendamento de transmissão:", error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}