...
function SubmitButton() {
 ...
 return (
 <button type="submit" disabled={pending} className="w-full bg-blue-600 text-white text-white font-bold py-4 px-6 rounded-lg hover:text-white hover: disabled:opacity-70 transition-all duration-300 hover:scale-[1.02] shadow-lg shadow-orange-500/20 text-lg uppercase tracking-wider flex items-center justify-center"
 >
 {pending ? (
...
export default function FormularioDeContatoBeta({ onClose }) {
 ...
 {/* Botão Fechar */}
 <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-white transition-colors bg-black/20 hover:bg-black/40 rounded-full p-2"
 aria-label="Fechar"
 >
 ...
 Interessado no <span className="text-blue-600">Beta Suítes</span>?
 </h2>
 ...
 className="w-full bg-black/30 border border-white/10 text-white rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-all placeholder-gray-500" />
 </div>
 ...
 <select onChange={(e) => setCountry(e.target.value)} value={country} className="appearance-none bg-black/30 border border-white/10 border-r-0 rounded-l-lg py-3 pl-3 pr-8 text-white focus:outline-none focus:ring-2 focus:ring-blue-600 cursor-pointer h-[50px]"
 >
 ...
 className="w-full bg-black/30 border border-white/10 border-l-0 text-white rounded-r-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent transition-all placeholder-gray-500 h-[50px]"
 />
 </div>
 </div>
