...
function SubmitButton() {
 ...
 return (
 <button type="submit" disabled={pending} className="w-full bg-blue-600 text-white font-bold py-4 px-6 rounded-lg hover:bg-blue-700 disabled:opacity-70 transition-all duration-300 shadow-lg uppercase tracking-wider flex items-center justify-center"
 >
 {pending ? (
...
export default function FormularioDeContato({ onClose }) {
 ...
 <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-blue-600 transition-colors bg-gray-100 hover:bg-blue-50 rounded-full p-2"
 aria-label="Fechar"
 >
 ...
 <select onChange={(e) => setCountry(e.target.value)} value={country} className="appearance-none bg-gray-50 border border-gray-300 border-r-0 rounded-l-lg py-3 pl-4 pr-8 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer h-[50px]"
 >
