...
function CorretorLayoutInner({ children }) {
 ...
 // Desktop: Se quiser implementar colapso depois, usaremos este estado. // Por enquanto, deixamos ele sempre expandido (false) para simplificar.
 const [isCollapsed, setIsCollapsed] = useState(false); const toggleSidebarMobile = () => setSidebarOpen(!isSidebarOpen);
 ...
 {/* === SIDEBAR WRAPPER === Correção: Adicionado 'w-64' e 'shrink-0' para garantir largura fixa no Desktop
 */}
 <div className={`
 fixed inset-y-0 left-0 z-30 w-64 shrink-0 bg-white border-r border-gray-200 h-full
 transform transition-transform duration-300 ease-in-out
 lg:translate-x-0 lg:static lg:inset-auto
 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}
 `}>
 <CorretorSidebar user={user} isUserLoading={isUserLoading} isCollapsed={isCollapsed} toggleSidebar={toggleSidebarDesktop} onMobileItemClick={() => setSidebarOpen(false)}
 />
 </div>
