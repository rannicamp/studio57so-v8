...
export default function ArquivosCorretorPage() {
 ...
 {mediaType === 'video' && anexo.public_url ? ( <div className="w-full h-full bg-black flex items-center justify-center"><video controls src={anexo.public_url} preload="metadata" className="w-full h-full max-h-full object-contain">Vídeo não suportado. <a href={anexo.public_url}>Assista aqui</a></video></div>
 ) : mediaType === 'pdf' && anexo.public_url ? ( <a href={anexo.public_url} target="_blank" rel="noopener noreferrer" className="w-full h-full relative flex items-center justify-center bg-white cursor-pointer hover:bg-red-50 transition-colors border-2 border-dashed border-transparent hover:border-red-200"> <iframe src={`${anexo.public_url}#toolbar=0&navpanes=0&scrollbar=0`} className="absolute inset-0 w-full h-full pointer-events-none opacity-40" title="PDF preview" /> <div className="z-10 bg-white/90 p-3 flex flex-col items-center justify-center rounded-lg shadow-sm border border-red-100 hover:scale-105 transition-transform"><FontAwesomeIcon icon={faFilePdf} className="text-red-500 text-3xl mb-1" /><span className="text-xs font-bold text-gray-700">Ver PDF</span></div> </a>
 ...
 ) : ( <a href={anexo.public_url} target="_blank" rel="noopener noreferrer" className="text-center text-gray-500 w-full h-full flex flex-col items-center justify-center hover:bg-gray-100 transition-colors"><FontAwesomeIcon icon={faFileAlt} size="3x" /><p className="text-xs mt-2 font-medium">Arquivo</p></a> )}
