...
export default function RecuperarSenhaPage() {
  ...
  const handleRecover = async (e) => {
    ...
    const res = await solicitarRecuperacaoSenhaAction(email, origin);

    setIsLoading(false);

    if (res.error) {
      console.error("Erro ao solicitar recuperação:", res.error);
      if (res.error.includes("rate limit") || res.error.includes("too many")) {
