...
const IGNORED_META_FIELDS = [
 'id', 'created_time', 'form_id', 'page_id', 'leadgen_id', 'ad_id', 'adgroup_id', 'campaign_id', 'platform', 'form_name',
...
export async function syncMetaFormsCatalog(organizacaoId) {
 ...
 .upsert(upsertData, { onConflict: 'organizacao_id,form_id',
...
export async function getMetaFormsList(organizacaoId) {
 ...
 .eq('status', 'ACTIVE') .order('last_synced', { ascending: false })
...
export async function getFormQuestions(organizacaoId, metaFormId) {
 ...
 return Object.keys(data.meta_form_data).filter(key => !IGNORED_META_FIELDS.includes(key) // APLICA O FILTRO
 )
 }
