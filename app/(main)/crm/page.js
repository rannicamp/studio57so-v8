...
export default function CrmPage() {
    ...
    const fetchWhatsappData = useCallback(async () => {
        ...
        try {
            // Modificação: Ajusta a busca para trazer a última mensagem e sua data
            ...
            const contatosComUltimaMensagem = data.map(contact => {
                let lastMessageContent = null;
                let lastMessageSentAt = null;

                if (contact.whatsapp_messages && contact.whatsapp_messages.length > 0) {
                    // Ordena as mensagens para encontrar a mais recente
                    const sortedMessages = [...contact.whatsapp_messages].sort((a, b) => new Date(b.sent_at).getTime() - new Date(a.sent_at).getTime());
                    lastMessageContent = sortedMessages[0].content;
                    lastMessageSentAt = sortedMessages[0].sent_at;
                }

                return {
                    ...contact,
                    last_whatsapp_message: lastMessageContent,
                    last_whatsapp_message_time: lastMessageSentAt,
                };
            });

            // Ordena os contatos pela data da última mensagem (mais recente primeiro)
            const sortedContatos = contatosComUltimaMensagem.sort((a, b) => {
                ...
                // Contatos com mensagens mais recentes vêm primeiro
                if (dateA && dateB) return dateB - dateA;
                if (dateA) return -1; // A tem mensagem e B não, A vem primeiro
                if (dateB) return 1;  // B tem mensagem e A não, B vem primeiro
                
                // Se nenhum tiver mensagem, ordena pelo nome
