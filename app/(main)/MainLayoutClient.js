...
// Componente interno com a lógica visual
function MainLayoutContent({ children }) {
    ...
    // Verificação de sessão (Client-side backup)
    useEffect(() => {
        if (!authLoading && !user) {
            console.error("Sessão fantasma detectada! Redirecionando...");
            // supabase.auth.signOut(); // Opcional: forçar logout
            router.push('/login');
        }
        ...
        <>
            {isGlobalActivityModalOpen && ( 
                <AtividadeModal 
                    ...
                    onActivityAdded={() => { setIsGlobalActivityModalOpen(false); toast.success('Atividade rápida criada com sucesso!'); }} 
                    activityToEdit={null} 
...
// Wrapper Principal que exportamos para o Layout do Servidor usar
