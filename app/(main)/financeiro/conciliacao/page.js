...
import { useQuery, useQueryClient } from '@tanstack/react-query'; import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faArrowLeft, faBank, faTrash, faCalendarAlt } from '@fortawesome/free-solid-svg-icons';
import Papa from 'papaparse'; import { toast } from 'sonner';
...
import LancamentoFormModal from '../../../../components/financeiro/LancamentoFormModal'; // --- HELPER: GERAÇÃO DE ID ÚNICO (ASSINATURA DIGITAL) ---
...
const parseCSV = (csvString) => {
 const results = Papa.parse(csvString, { header: true, skipEmptyLines: true, transformHeader: h => h.trim() });
 ...
 return {
 id: finalId,
 data: dataFormatada, descricao: descricao,
 valor: valor,
 tipo: valorOriginal < 0 ? 'DEBIT' : 'CREDIT', conciliado: false
 };
...
export default function ConciliacaoPage() {
 ...
 const [extratoItems, setExtratoItems] = useState([]); // Filtros
 const [dataInicio, setDataInicio] = useState(format(startOfMonth(new Date()), 'yyyy-MM-dd'));
 const [dataFim, setDataFim] = useState(format(endOfMonth(new Date()), 'yyyy-MM-dd'));
 const [isLoaded, setIsLoaded] = useState(false); // Edição/Criação
 ...
 } catch (e) { console.error("Erro sessão:", e); } finally { setIsLoaded(true); }
 }, []);
 ...
 let query = supabase
 .from('lancamentos')
 .select('*, fitid') .eq('conta_id', contaSelecionada.id)
 ...
 valor: d.valor,
 tipo: d.tipo, conciliado: d.status === 'Conciliado' || d.conciliado === true };
 }) || [];
 },
 ...
 const datasOrdenadas = parsed.map(p => p.data).sort((a, b) => a.localeCompare(b)); if (datasOrdenadas.length > 0) {
 ...
 data_transacao: dataExtrato, data_vencimento: dataExtrato, data_pagamento: isCartao ? null : dataExtrato,
 status: isCartao ? 'Pendente' : 'Conciliado', conciliado: true,
 fitid: itemBanco.id, auditoria_verificado: true
 };
 ...
 if (!isLoaded) return null; return (
 ...
 <div className="w-64">
 <select className="w-full p-2 border border-gray-300 rounded-lg text-sm bg-white shadow-sm focus:ring-2 focus:ring-blue-500 outline-none"
 onChange={(e) => {
