...
import { 
  faUsers, faBirthdayCake, faUserPlus, faUserMinus, 
  faMoneyBillWave, faSpinner, faCalendarCheck, faClock, 
  faExclamationTriangle
} from '@fortawesome/free-solid-svg-icons';
import { createClient } from '@/utils/supabase/client';
import { useAuth } from '@/contexts/AuthContext';
// IMPORTANTE: Importando o componente isolado
...
export default function RelatorioRhPage() {
  ...
  const formatTempoCasa = (meses) => {
    ...
    return anos > 0 ? `${anos} ano(s) e ${m} mês(es)` : `${m} meses`;
  };
  ...
  const pieOptions = {
    title: "Distribuição por Cargo (Mês Selecionado)",
    pieHole: 0.4,
    is3D: false,
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-sm text-gray-500">Visão consolidada do mês</p>
        </div>
        ...
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="col-span-1 md:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-500 mt-2">
                    <span className="px-2 py-1 bg-gray-100 rounded text-gray-600 font-medium"><FontAwesomeIcon icon={faCalendarCheck} className="mr-1" />{stats?.dias_uteis_mes || 0} dias úteis</span>
                </div>
            </div>
            ...
            <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 border-dashed flex flex-col items-center justify-center text-center">
                ...
                {stats?.total_ativos > 0 ? (((Number(stats.demissoes)+Number(stats.admissoes))/2/Number(stats.total_ativos))*100).toFixed(1)+'%' : '0%'}
                </span>
            </div>
        </div>

        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col min-h-[400px]">
            <h4 className="text-sm font-semibold text-gray-600 mb-4 border-b pb-2">Cargos no mês</h4>
            <div className="flex-1 flex items-center justify-center">
              {loadingStats ? (
                ...
                <div className="text-center text-gray-400 text-sm">Sem dados.</div>
              )}
            </div>
        </div>
      </div>

      {/* Seção Isolada do Gráfico */}
      <EvolutionChart />
