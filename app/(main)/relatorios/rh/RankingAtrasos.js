...
async function fetchAtrasos(organizacao_id, dateRef) {
 ...
 const { data, error } = await supabase.rpc('calcular_ranking_atrasos', { p_organizacao_id: organizacao_id, p_mes_ref: dateRef ? dateRef.slice(0, 7) : null
 });
...
export default function RankingAtrasos({ mesRef }) {
 ...
 if (!mesRef) return '';
 const [ano, mes] = mesRef.split('-'); const ultimoDia = new Date(ano, mes, 0).getDate();
 return `01/${mes}/${ano} a ${ultimoDia}/${mes}/${ano}`;
 };

 return (
 <RankingWidget title="Mais Atrasos"
 icon={faUserClock}
 color="orange"
 labelUnit="atrasos"
 data={data}
