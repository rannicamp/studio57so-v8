...
async function fetchFaltas(organizacao_id, dateRef) {
 ...
 const { data, error } = await supabase.rpc('calcular_ranking_faltas', { p_organizacao_id: organizacao_id, p_mes_ref: dateRef ? dateRef.slice(0, 7) : null
 });
...
export default function RankingFaltas({ mesRef }) {
 ...
 return (
 <RankingWidget title="Mais Faltas"
 icon={faUserSlash}
 color="red"
 labelUnit="faltas"
 data={data}
