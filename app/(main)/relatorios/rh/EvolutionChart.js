...
import { faChartLine, faSpinner, faExclamationTriangle
...
async function fetchRhYearlyStats(organizacao_id, ano) {
 ...
 const { data, error } = await supabase.rpc('get_rh_yearly_stats', { p_organizacao_id: organizacao_id, p_ano: ano });
...
export default function EvolutionChart() {
 ...
 return { dataset: processed, maxVal: maxVal > 0 ? maxVal : 10, // Escala mínima de 10 para não quebrar visual
 ...
 {media > 0 && (
 <div className="absolute left-8 right-2 border-t-2 border-red-500 border-dashed z-30 pointer-events-none transition-all duration-1000 flex justify-end"
 ...
 <div className="w-full bg-emerald-200 border border-emerald-300 rounded-t-sm absolute bottom-0 flex items-end justify-center transition-all duration-500 group-hover:bg-emerald-300"
 ...
 {data.admissoes > 0 && (
 <div className="w-1/2 bg-indigo-600 rounded-t-sm z-10 transition-all duration-500 hover:bg-indigo-700 absolute bottom-0 left-0 shadow-sm border-r border-white/20"
 ...
 {data.demissoes > 0 && (
 <div className="w-1/2 bg-red-500 rounded-t-sm z-10 transition-all duration-500 hover:bg-red-600 absolute bottom-0 right-0 shadow-sm border-l border-white/20"
