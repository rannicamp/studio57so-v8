...
const TabButton = ({ label, activeTab, onClick }) => (
 <button
 onClick={onClick}
 className={`px-4 py-3 text-lg font-semibold border-b-4 transition-colors duration-300 ${activeTab === label ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
 >
 {label}
 </button>
);
...
export default function FerramentasContatoPage() {
 ...
 <TabButton label="Vincular Funcionários" activeTab={activeTab}
 onClick={() => setActiveTab('Vincular Funcionários')}
 />
 <TabButton label="Mesclar Contatos Duplicados" activeTab={activeTab}
