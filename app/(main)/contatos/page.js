...
import {
 ...
 faFileImport, faFileExport, faLayerGroup, faObjectGroup, faWandMagicSparkles, faFilter,
...
async function fetchContatosMain(organizacaoId, searchTerm, typeFilter) {
 ...
 if (error) { console.error("Erro busca contatos:", error.message); throw new Error(error.message) }
...
export default function ContatosMain() {
 ...
 const hasRestoredUiState = useRef(true); const isInitialMount = useRef(true);
 ...
 setIsModalOpen(false); setContatoParaEditar(null); queryClient.invalidateQueries({ queryKey: ['contatosMainLista'] });
 }, [queryClient]);
 ...
 ) : (<FontAwesomeIcon icon={cliente.personalidade_juridica === 'Pessoa Jurídica' ? faBuilding : faUserCircle} className={`w-10 h-10 rounded-full p-2 flex-shrink-0 ${ cliente.personalidade_juridica === 'Pessoa Jurídica' ? 'bg-blue-600 text-blue-600' : 'bg-blue-100 text-blue-600' }`} /> )}
 </td>
 ...
 <ContatoDetalhesSidebar contactId={sidebarContactId} onClose={() => setSidebarContactId(null)} />
 </div>
 )
}