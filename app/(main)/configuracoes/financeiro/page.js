...
import { faTags, faFileImport, faHandshake, faArrowLeft, faShieldAlt,
 faSpinner,
 faLock
...
export default function ConfigFinanceiroPage() {
 ...
 <button onClick={() => setActiveTab(tabName)} className={`whitespace-nowrap py-4 px-6 border-b-2 font-medium text-sm flex items-center gap-2 transition-colors ${
 ...
 > <FontAwesomeIcon icon={icon} className={activeTab === tabName ? 'text-blue-500' : 'text-gray-400'} /> {label} </button>
 );
