-- supabase/migrations/20260707_recursive_bim_mappings.sql

DROP FUNCTION IF EXISTS public.get_quantitativos_orcamentacao_bim(bigint, bigint, bigint[]);
DROP FUNCTION IF EXISTS public.get_quantitativos_por_categoria_bim(bigint, bigint, bigint[]);

-- 1. Recriando get_quantitativos_orcamentacao_bim com suporte recursivo
...
RETURNS TABLE(
    ...
    pai_material_id bigint,
    pai_sinapi_id bigint,
    nivel bigint
) AS $$
BEGIN
    RETURN QUERY
    WITH RECURSIVE elementos AS (
        SELECT 
            e.id,
            e.external_id,
            e.categoria,
    ...
    vinculos_nativos_filtrados AS (
        SELECT DISTINCT ON (v.elemento_id, COALESCE(NULLIF(v.matching_key, '@ELEMENTO@'), '!GLOBAL'))
            ...
            NULL::bigint AS pai_material_id,
            NULL::bigint AS pai_sinapi_id
    ...
    vinculos_filhos AS (
        -- Âncora: Filhos imediatos (nível 2)
        SELECT
            tp.elemento_id,
            tp.external_id,
            tp.categoria,
            ...
            tp.material_id AS pai_material_id,
            tp.sinapi_id AS pai_sinapi_id,
            2::bigint AS nivel
        FROM todos_pais tp
        JOIN mapeamentos mf ON mf.vinculo_pai_id = tp.mapeamento_id
        
        UNION ALL
        
        -- Recursão: Netos, Bisnetos, etc. (nível 3+)
        SELECT
            vf.elemento_id,
            vf.external_id,
            vf.categoria,
            ...
            false AS is_avulso,
            mf.vinculo_pai_id AS pai_mapeamento_id,
            vf.material_id AS pai_material_id,
            vf.sinapi_id AS pai_sinapi_id,
            (vf.nivel + 1)::bigint AS nivel
        FROM vinculos_filhos vf
        JOIN mapeamentos mf ON mf.vinculo_pai_id = vf.mapeamento_id
    ),
    todos_vinculos AS (
        SELECT vnf.*, 1::bigint AS nivel FROM vinculos_nativos_filtrados vnf
        UNION ALL
        SELECT vf.* FROM vinculos_filhos vf
        UNION ALL
        SELECT va.*, 1::bigint AS nivel FROM vinculos_avulsos va
    ),
    agregado AS (
        SELECT 
            ...
            MAX(m.pai_material_id) AS pai_material_id,
            MAX(m.pai_sinapi_id) AS pai_sinapi_id,
            MAX(m.nivel) AS nivel
...
-- 2. Recriando get_quantitativos_por_categoria_bim com suporte recursivo
CREATE OR REPLACE FUNCTION public.get_quantitativos_por_categoria_bim(
    p_organizacao_id bigint,
    p_empreendimento_id bigint,
    p_projeto_ids bigint[] DEFAULT NULL::bigint[]
)
...
BEGIN
    ...
    vinculos_filhos AS (
        ...
        SELECT
            ...
            false AS is_avulso,
            mf.vinculo_pai_id AS pai_mapeamento_id,
            vnf.material_id AS pai_material_id,
            vnf.sinapi_id AS pai_sinapi_id,
            2::bigint AS nivel
    ...
    agregado AS (
        SELECT 
            COALESCE(NULLIF(TRIM(m.categoria), ''), 'Materiais do Projeto') AS categoria_nome,
            (CASE WHEN m.material_id IS NOT NULL THEN 'mat_' || m.material_id ELSE 'sinapi_' || m.sinapi_id END) || 
            '_et' || COALESCE(m.etapa_id::text, '0') || 
            '_sub' || COALESCE(m.subetapa_id::text, '0') || 
            '_pai' || COALESCE(m.pai_mapeamento_id::text, '0') ||
            ...
            MAX(m.nivel) AS nivel
        FROM todos_vinculos m
        GROUP BY COALESCE(NULLIF(TRIM(m.categoria), ''), 'Materiais do Projeto'), m.material_id, m.sinapi_id, m.etapa_id, m.subetapa_id, m.pai_mapeamento_id, m.is_avulso
    )
    SELECT 
        a.categoria_nome::text AS categoria_nome,
        a.key_id::text AS key,
        a.mapeamento_id,
        COALESCE(mat.nome, sin.nome) AS nome,
        COALESCE(
    ...
    LEFT JOIN public.materiais mat ON a.material_id = mat.id
    LEFT JOIN public.sinapi sin ON a.sinapi_id = sin.id
    ORDER BY a.categoria_nome, custo_total DESC;
END;
$$ LANGUAGE plpgsql;
