-- Migration para Erradicação Sistemática de Notificações Duplicadas (Ticket #76)

BEGIN;

-- 1. Remoção de Gatilhos Fantasmas / Duplicados (Legado)
-- Foi identificado que o sistema auto-gerenciava triggers (trg_auto_notificacao_*) mas antigos (manuais) ainda operavam.
DROP TRIGGER IF EXISTS trg_notificacao_activities ON public.activities;
DROP TRIGGER IF EXISTS tr_processar_regras_notificacao_contatos ON public.contatos;
DROP TRIGGER IF EXISTS trg_gerenciar_notificacoes ON public.sys_notification_templates;

-- 2. Reescrevendo a Função Master com Bloqueio de Duplicidade (Memória Array)
        ...
        BEGIN 
            ...
            IF r_regra.funcoes_ids IS NOT NULL AND array_length(r_regra.funcoes_ids, 1) > 0 THEN
                FOR r_user IN SELECT id FROM public.usuarios WHERE funcao_id::text = ANY(r_regra.funcoes_ids::text[]) AND organizacao_id = NEW.organizacao_id LOOP
                    ...
                    -- Registrar que o usuário já recebeu essa notificação
            ...
            IF r_regra.enviar_para_dono = true THEN
                ...
                -- Condição restrita: Se ele NÃO ESTIVER na lista de notificados
