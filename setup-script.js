...
BEGIN
    ...
    conversas_whatsapp AS (
        SELECT 
            c.id AS contato_id,
            c.origem,
            (SELECT usuario_responsavel_id FROM contatos_no_funil WHERE contato_id::text = c.id::text LIMIT 1) as usuario_responsavel_id,
    ...
    metricas_corretores AS (
        SELECT 
            COALESCE(u.nome, 'Sem Corretor / Robô') as corretor_nome,
            COUNT(DISTINCT cw.contato_id) as total_atendimentos,
            COALESCE(AVG(EXTRACT(EPOCH FROM (cw.msg_primeiro_outbound - cw.contato_criado_em))/60) FILTER (WHERE cw.msg_primeiro_outbound > cw.contato_criado_em), 0) as tempo_medio_resposta_minutos,
        ...
        LEFT JOIN (
            SELECT usuario_responsavel_id, coluna_nome, COUNT(*) as qtd
            FROM cards_do_periodo
            GROUP BY usuario_responsavel_id, coluna_nome
        ) dist ON dist.usuario_responsavel_id::text = cw.usuario_responsavel_id::text
        LEFT JOIN auth_users_view u ON u.id::text = cw.usuario_responsavel_id::text
        GROUP BY u.nome
...
$function$;
     ...
     console.log("Sucesso! RPC recriada sem a coluna fantasma.");
  } catch (e) {
