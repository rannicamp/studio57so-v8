// scratch/investigar_lead_hudson.js
...
const TELEFONE_BUSCA = '553388157890';

async function main() {
  console.log(`=== INVESTIGANDO LEAD DO WHATSAPP: Hudson Mota (${TELEFONE_BUSCA}) ===`);
  ...
  console.log(`Telefones possíveis para busca:`, [cleanPhone, cleanPhoneSem9, cleanPhoneCom9]);
  ...
  if (errT) {
    console.error("Erro ao buscar tabela telefones:", errT.message);
    return;
  }
  ...
  // 3. Buscar histórico de mensagens da conversa (incluindo status e payloads de erro)
  console.log("\n3. Buscando histórico de mensagens da conversa...");
  const { data: mensagens, error: errM } = await supabase
    .from('whatsapp_messages')
    .select('id, direction, content, created_at, sent_at, status, error_message, raw_payload')
  ...
  if (mensagens && mensagens.length > 0) {
    mensagens.forEach(msg => {
      ...
      const statusStr = msg.status ? ` [Status: ${msg.status}]` : '';
      const erroStr = msg.error_message ? ` (ERRO: ${msg.error_message})` : '';
      console.log(`[${dataFormatada}] ${remetenteStr}: "${msg.content}"${statusStr}${erroStr}`);
    });
  } else {
