...
async function runSQL() {
  ...
  if (!connStr) { 
      console.error('ERRO FATAL: SUPABASE_DATABASE_URL não encontrada na .env.local.'); 
      return; 
  }
  
  // O pooler do Supabase no env.local muitas vezes usa session mode ou transaction mode. 
  // Na porta 6543 do IPv4, ele deve conectar ok. Vamos tentar o connStr direto.
  const client = new Client({ connectionString: connStr });
  
  try {
     console.log("Estabelecendo link com Supabase via URL do env...");
     ...
     console.log("🔥 Triggers Encontrados antes da Deleção:", resTriggers.rows);
     ...
     // Dropa apenas para limpar, caso exista
