---
description: Mapeamento oficial das regras de qualificacao consultiva, transbordo de funil e matriz de eventos e acoes comerciais da Stella IA SDR 2.0.
---
# Workflow / Manual Operacional: Algoritmo Stella IA SDR 2.0

Este documento descreve o fluxo lógico de decisão e a matriz de eventos da **Stella IA SDR 2.0** no CRM do Studio 57/Elo 57. Ele deve ser lido e seguido por qualquer assistente de IA que precise ajustar ou manter o código da Stella.

---

## 📊 Matriz de Eventos, Situações e Ações Comerciais

A tabela abaixo descreve de forma estruturada qual ação a IA ou o sistema executa diante de cada situação específica na conversa:
...
| **8** | **O cliente responde com evasivas ou desinteresse/recusa explícita** | Frases do tipo "só olhando", "não sei", "depois" OU recusa direta (ex: clique no botão "Não, Obrigado!", mensagens como "não tenho interesse", "não quero") | IA move o lead para a coluna **PERDIDO** (ID: `feaa8511-261d-451b-bf99-24c8a6d6e7e0`), envia mensagem polida de encerramento, desativa o piloto automático e o insere em fluxo automático de descarte. | Supabase (CRM) & WhatsApp |
| **9** | **Falha de entrega na API de mensagens da Meta** | Status da mensagem retorna `failed` no Webhook | O sistema desativa o piloto automático, move o lead para a coluna **FALHAS** (ID: `2b975bc0-b96c-456d-ac30-48ab6f6dddca`) e cria uma nota no CRM relatando o problema. | Supabase (CRM) |
| **10**| **Cliente silenciado responde de forma voluntária** | Piloto automático estava falso na coluna de FALHAS | O sistema reativa o piloto automático, move para **EM ATENDIMENTO** (ID: `029c8d6a-4799-4f4b-a55e-b4d5426718c0`) e cria nota de auto-recuperação no CRM. | Supabase (CRM) |
...
O algoritmo de decisão da IA segue a seguinte sequência lógica a cada iteração:

```mermaid
graph TD
    A[Mensagem Inbound Recebida] --> B{Tem mensagens anteriores?}
    B -- Não --> C[Saudação com Disclaimer + Apresenta Produto]
    B -- Sim --> D[Identificar interesse em Empreendimento específico]
    D --> E{Solicitou valores ou simulação?}
    E -- Sim --> F[Dizer valor a partir de + Perguntar Renda e FGTS para simular]
    E -- Não --> G[Sondar Objetivo de Compra: Moradia, Lazer ou Investimento]
    F --> H{Forneceu dados financeiros?}
    H -- Sim --> I{Qualificação está 100% concluída?}
    I -- Sim --> K[Enviar Mensagem de Passagem de Bastão + Mover para QUALIFICAÇÃO STELLA + Criar Handoff no CRM + Desativar IA]
    I -- Não --> L[Apenas Atualizar Ficha + Continuar Perguntando + mover_para_coluna_id: null]
    H -- Não --> L
    G --> M{Qualificou Perfil de Uso?}
    M -- Sim --> I
    M -- Não --> L
```
