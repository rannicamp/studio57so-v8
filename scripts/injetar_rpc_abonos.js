// scripts/injetar_rpc_abonos.js
// USO: node scripts/injetar_rpc_abonos.js SUA_SENHA_DO_BANCO
...
async function run() {
    ...
    try {
        ...
        const sqlContent = fs.readFileSync('db_rpc_abonos.sql', 'utf8');
        const cleanSQL = sqlContent.split('\n').filter(l => !l.trim().startsWith('--')).join('\n').trim();

        console.log('⚙️  Injetando RPC get_rh_tendencia_abonos...');
        await client.query(cleanSQL);
        console.log('✅ RPC criada com sucesso!\n');

        console.log('🧪 Testando a RPC...');
        const { rows } = await client.query("SELECT get_rh_tendencia_abonos('2026', 1) as resultado");
