...
async function run() {
  ...
  if (data.data) {
      const templatesInfo = data.data.map(t => ({ name: t.name, language: t.language, status: t.status }));
      console.log("=== TEMPLATES REGISTRADOS NA META ===");
      console.table(templatesInfo);
  } else {
