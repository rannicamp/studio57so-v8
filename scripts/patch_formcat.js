...
// 1. Add passivosDisponiveis to props
c = c.replace(
  'ativosDisponiveis = [], contratosDisponiveis = []',
  'ativosDisponiveis = [], contratosDisponiveis = [], passivosDisponiveis = []'
);

// 2. Add passivos block
const blockAtivo = ` </select>
 </div>
 )}`;
...
// Replace the LAST occurrence of blockAtivo before the observacoes textarea
const parts = c.split(blockAtivo);
if(parts.length > 1) {
    // Join the last piece with the rest
    const lastPiece = parts.pop();
    c = parts.join(blockAtivo) + blockAtivo + blockPassivo + lastPiece;
}
